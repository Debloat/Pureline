...
bool BuildMobTable()
{
    ...
    {
        fprintf (stderr, "Failed to read mob_names.txt file\n");
        isNameFile = false;
    }
    else
    {
    ...
    {
        fprintf (stderr, "Failed to read mob_proto_test.txt file\n");
        //return false;
    }
    else
    {
    ...
    {
        fprintf (stderr, "Failed to read mob_proto.txt file\n");
        return false;
    }
    ...
    else
    {
        ...
        while (test_data.Next())    //테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
        {
            ...
            fprintf (stdout, "[New]MOB #%-5d %-16s sight: %u color %u[%s]\n", mob_table->dwVnum, mob_table->szName, mob_table->wAggressiveSight, mob_table->dwMobColor,
...
bool BuildItemTable()
{
    ...
    {
        fprintf (stderr, "Failed to read file item_names.txt\n");
        isNameFile = false;
    }
    else
    {
    ...
    {
        fprintf (stderr, "Failed to read file item_proto_test.txt\n");
        //return false;
    }
    else
    {
    ...
    {
        fprintf (stderr, "Failed to read file item_proto.txt\n");
        return false;
    }
    data.Next(); //맨 윗줄 제외 (아이템 칼럼을 설명하는 부분)
