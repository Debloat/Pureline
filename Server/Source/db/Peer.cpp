...
DWORD CPeer::GetHandle() const
{
    return m_dwHandle;
}

DWORD CPeer::GetUserCount() const
{
...
bool CPeer::PeekPacket(int& iBytesProceed, BYTE & header, DWORD & dwHandle, DWORD & dwLength, const char** data)
{
    ...
    const auto* buf = (const char*) GetRecvBuffer();
    buf += iBytesProceed;
...
bool CPeer::CheckItemIDRangeCollision(TItemIDRangeTable itemRange) const
{
