...
bool CClientManager::InitializeMobTable()
{
	...
	while (data.Next())
	{
		...
		sys_log(1, "MOB #%-5d %-24s level: %-3u rank: %u empire: %d", mob_table->dwVnum, mob_table->szName, mob_table->bLevel, mob_table->bRank,
			mob_table->bEmpire);
		++mob_table;
	...
	if (isTestFile)
	{
		...
		while (test_data.Next())	//테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
		{
			...
			sys_log(0, "MOB #%-5d %-24s level: %-3u rank: %u empire: %d", mob_table->dwVnum, mob_table->szName, mob_table->bLevel, mob_table->bRank,
				mob_table->bEmpire);
			++mob_table;

		}
	}
	std::ranges::sort(m_vec_mobTable, FCompareVnum());
	return true;
}
...
bool CClientManager::InitializeShopTable()
{
	...
	while ((data = mysql_fetch_row(pRes2->pSQLResult)))
	{
		...
		if (!map_shop.contains(iShopVnum))
		{
...
bool CClientManager::InitializeItemTable()
{
	...
	std::ranges::sort(m_vec_itemTable, FCompareVnum());
	return true;
}
...
static constexpr std::array<const char*, 3> EMobSizeNames =
{
	"SMALL",
	"MEDIUM",
	"BIG"
...
bool CClientManager::MirrorMobTableIntoDB()
{
	for (auto it = m_vec_mobTable.begin(); it != m_vec_mobTable.end(); it++)
	{
		...
		snprintf(query, sizeof(query),
			...
			t.dwVnum, t.szName, t.bType, t.bRank, t.bBattleType, t.bLevel, EMobSizeNames[t.bSize], t.dwAIFlag, t.dwRaceFlag, t.dwImmuneFlag,
