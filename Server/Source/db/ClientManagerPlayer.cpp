...
void CClientManager::QUERY_PLAYER_LOAD (CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
    ...
    //
    if (CLoginData * pLoginData = GetLoginDataByAID (packet->account_id))
    {
...
{
    auto * qi = (CQueryInfo*) pMsg->pvUserData;
...
using time_by_id_map_t = std::map<DWORD, time_t>;
...
void CClientManager::__QUERY_PLAYER_CREATE (CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
    ...
    if (time_by_id_map_t::iterator it = s_createTimeByAccountID.find (packet->account_id); it != s_createTimeByAccountID.end())
    {
    ...
    if (std::unique_ptr<SQLMsg> pMsg0 (CDBManager::instance().DirectQuery (queryStr)); pMsg0->Get()->uiNumRows != 0)
    {
    ...
    if (std::unique_ptr<SQLMsg> pMsg1 (CDBManager::instance().DirectQuery (queryStr)); pMsg1->Get()->uiNumRows)
    {
    ...
    queryLen = snprintf (queryStr, sizeof (queryStr),
                         ...
                         packet->player_table.hp, packet->player_table.sp, packet->player_table.sRandomHP, packet->player_table.sRandomSP, packet->player_table.stat_point,
                         packet->player_table.stamina, packet->player_table.part_base, packet->player_table.part_base, packet->player_table.gold);
    ...
    if (std::unique_ptr<SQLMsg> pMsg3 (CDBManager::instance().DirectQuery (queryStr)); pMsg3->Get()->uiAffectedRows <= 0)
    {
...
void CClientManager::__RESULT_PLAYER_DELETE (CPeer *peer, SQLMsg* msg)
{
    ...
    if (msg->Get() && msg->Get()->uiNumRows)
    {
        ...
        if (std::unique_ptr<SQLMsg> pIns (CDBManager::instance().DirectQuery (queryStr)); pIns->Get()->uiAffectedRows == 0 || pIns->Get()->uiAffectedRows == (uint32_t) -1)
        {
        ...
        // 플레이어 테이블을 캐쉬에서 삭제한다.
        if (CPlayerTableCache * pkPlayerCache = GetPlayerCache (pi->player_id))
        {
        ...
        // 아이템들을 캐쉬에서 삭제한다.
        if (TItemCacheSet * pSet = GetItemCacheSet (pi->player_id))
        {
            auto it = pSet->begin();
        ...
        if (std::unique_ptr<SQLMsg> pMsg (CDBManager::instance().DirectQuery (queryStr)); pMsg->Get()->uiAffectedRows == 0 || pMsg->Get()->uiAffectedRows == (uint32_t) -1)
        {
        ...
        snprintf (queryStr, sizeof (queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (window < %d or window = %d)", GetTablePostfix(), pi->player_id, SAFEBOX,
...
void CClientManager::QUERY_HIGHSCORE_REGISTER (CPeer* peer, TPacketGDHighscore * data)
{
    ...
    auto * pi = new ClientHandleInfo (0);
...
{
    // 존재하지 않을경우 추가
    if (TLogoutPlayerMap::iterator it = m_map_logout.find (pid); it != m_map_logout.end())
    {
        // 존재할경우 시간만 갱신
        if (g_log)
        {
