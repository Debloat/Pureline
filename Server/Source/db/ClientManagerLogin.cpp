...
void CClientManager::RESULT_LOGIN_BY_KEY (CPeer * peer, SQLMsg * msg)
{
    ...
    auto * info = (ClientHandleInfo*) qi->pvData;
...
{
    auto* packet = (TLogoutPacket*)data;
...
void CClientManager::QUERY_CHANGE_NAME (CPeer * peer, DWORD dwHandle, TPacketGDChangeName * p)
{
    ...
    if (std::unique_ptr<SQLMsg> pMsg (CDBManager::instance().DirectQuery (queryStr, SQL_PLAYER)); pMsg->Get()->uiNumRows)
    {
