...
bool CClientManager::Initialize()
{
    ...
    {
        std::println(stderr, "Failed Localization Infomation so exit");
        return false;
    }
    ...
    {
        std::println(stderr, " Item range Initialize Failed. Exit DBCache Server");
        return false;
    }
    //END_ITEM_UNIQUE_ID
    ...
    if (int iChinaEventServer = 0; CConfig::instance().GetValue("CHINA_EVENT_SERVER", &iChinaEventServer))
    {
...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
    ...
    if (pi->account_index == 0)
    {
        ...
        if (SQLResult * res = msg->Get(); res->uiNumRows == 0)
        {
    ...
    else
    {
        ...
        if (std::set<TItemAward*>* pSet = ItemAwardManager::instance().GetByLogin(pi->login); pSet && !m_vec_itemTable.empty())
        {
                        ...
                        if (0 == pItemAward->dwSocket0)
                        {
                            for (int i = 0; i < ITEM_LIMIT_MAX_NUM; i++)
                            {
                                if (LIMIT_REAL_TIME == item_table->aLimits[i].bType)
                                {
                                    ...
                                    {
                                        pItemAward->dwSocket0 = time(nullptr) + 60 * 60 * 24 * 7;
                                    }
                                    else
                                    {
                                        pItemAward->dwSocket0 = time(nullptr) + item_table->aLimits[i].lValue;
                                    }
...
{
    auto * qi = (CQueryInfo*) msg->pvUserData;
    auto * p = (ClientHandleInfo*) qi->pvData;
...
{
    auto* pReqInfo = (TItemPricelistReqInfo*)static_cast<CQueryInfo*> (pMsg->pvUserData)->pvData;
...
{
    auto* pUpdateTable = (TItemPriceListTable*)static_cast<CQueryInfo*> (pMsg->pvUserData)->pvData;
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char* c_pData)
{
    ...
    // 로그인 및 빌링정보 보내기
    //
    auto * pck = (TPacketLoginOnSetup*) c_pData;;
...
void CClientManager::DeleteObject(DWORD dwID)
{
    ...
    if (std::unique_ptr<SQLMsg> pmsg(CDBManager::instance().DirectQuery(szQuery)); pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t) -1)
    {
        sys_err("no object by id %u", dwID);
        return;
    }

    if (auto it = m_map_pkObjectTable.find(dwID); it != m_map_pkObjectTable.end())
    {
        delete it->second;
...
int CClientManager::Process()
{
    ...
    for (int idx = 0; idx < num_events; ++idx) // 인풋
    {
...
bool CClientManager::InitializeLocalization()
{
    ...
    for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
    {
        ...
        {
            if (strcmp(locale.szValue, "ymir") == 0)
            {
            ...
            else if (strcmp(locale.szValue, "multilanguage") == 0)
            {
                sys_log(0, "locale[LOCALE] = %s", locale.szValue);

                if (g_stLocale != locale.szValue)
                {
                    sys_log(0, "Changed g_stLocale %s to %s", g_stLocale.c_str(), "multilanguage");
                }
...
void CClientManager::DeleteAwardId(TPacketDeleteAwardID *data)
{
    ...
    std::map<DWORD, TItemAward*>::iterator it = ItemAwardManager::Instance().GetMapAward().find(data->dwID);
    if (it != ItemAwardManager::Instance().GetMapAward().end())
    {
        if (std::set<TItemAward*>& kSet = ItemAwardManager::Instance().GetMapkSetAwardByLogin()[it->second->szLogin]; kSet.erase(it->second))
        {
