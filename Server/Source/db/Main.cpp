...
static void emptybeat(LPHEART heart, int pulse)
{
...
int Start()
{
    ...
    {
        std::println(stderr, "Loading conf.txt failed.");
        return false;
    }

    if (!CConfig::instance().GetValue("TEST_SERVER", &g_test_server))
    {
        std::println(stderr, "Real Server");
    }
    else
    {
        std::println(stderr, "Test Server");
    }

    if (!CConfig::instance().GetValue("LOG", &g_log))
    {
        std::println(stderr, "Log Off");
        g_log = 0;
    }
    else
    {
        g_log = 1;
        std::println(stderr, "Log On");
    }
    ...
    {
        std::println(stderr, "Cannot find CLIENT_HEART_FPS configuration.");
        return false;
    }
    ...
    if (CConfig::instance().GetValue("LOG_KEEP_DAYS", &tmpValue))
    {
        ...
        std::println(stderr, "Setting log keeping days to %d", tmpValue);
    }
    ...
    if (CConfig::instance().GetValue("SQL_PLAYER", line, 256))
    {
        ...
        do
        {
            ...
            std::println(stderr, "   failed, retrying in 5 seconds");
            sleep(5);
        }
        while (iRetry--);
        std::println(stderr, "Success PLAYER");
    ...
    if (CConfig::instance().GetValue("SQL_ACCOUNT", line, 256))
    {
        ...
        while (iRetry--);
        std::println(stderr, "Success ACCOUNT\n");
    }
    else
    {
    ...
    if (CConfig::instance().GetValue("SQL_COMMON", line, 256))
    {
        ...
        while (iRetry--);
        std::println(stderr, "Success COMMON");
    }
    else
    {
