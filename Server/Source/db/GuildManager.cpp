...
bool isEurope()
{
    do
    {
        ...
        if (g_stLocale.compare("multilanguage") == 0)
        {
            break;
        }
...
TGuild& CGuildManager::TouchGuild(DWORD GID)
{
    ...
    TGuild info;
    m_map_kGuild.try_emplace(GID, info);
...
void CGuildManager::ParseResult(SQLResult * pRes)
{
    ...
    {
        DWORD GID = strtoul(row[0], nullptr, 10);
...
void CGuildManager::Initialize()
{
    ...
    if (std::unique_ptr<SQLMsg> pmsg(CDBManager::instance().DirectQuery(szQuery)); pmsg->Get()->uiNumRows)
    {
...
bool CGuildManager::IsHalfWinLadderPoint(DWORD dwGuildWinner, DWORD dwGuildLoser)
{
    ...
    if (auto it = m_mapGuildWarEndTime[GID1].find(GID2); it != m_mapGuildWarEndTime[GID1].end() &&
...
void CGuildManager::ProcessWinLose(DWORD dwGuildWinner, DWORD dwGuildLoser)
{
    ...
    auto gain = (int)(iPoint * 0.05);
    auto loss = (int)(iPoint * 0.07);
...
{
    if (TGuildDeclareInfo di(bType, guild_from, guild_to); !m_DeclareMap.contains(di))
    {
...
void CGuildManager::BootReserveWar()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        while ((row = mysql_fetch_row(pmsg->Get()->pSQLResult)))
        {
            ...
            auto * pkReserve = new CGuildWarReserve(t);
        ...
        if (!r.bStarted && r.dwTime - 1800 <= dwCurTime) // 30분 전부터 알린다.
        {
            ...
            else
            {
                if (iMin != pk->GetLastNoticeMin())
                {
                    ...
                    CClientManager::instance().SendNotice("The war between the %s guild and the %s guild will begin in approximately %d minutes!", r_1.szName, r_2.szName, iMin);
                }
            }
        }
    }
