...
void CClientManager::GuildAddMember(CPeer* peer, TPacketGDGuildAddMember * p)
{
    ...
    snprintf(szQuery, sizeof(szQuery),
             "SELECT pid, grade, is_general, offer, level, job, name FROM guild_member%s, player%s WHERE guild_id = %u and pid = id and pid = %u", GetTablePostfix(), GetTablePostfix(),
             p->dwGuild, p->dwPID);
...
void CClientManager::GuildRemoveMember(CPeer* peer, TPacketGuild* p)
{
    ...
    snprintf(szQuery, sizeof(szQuery), "REPLACE INTO quest%s (dwPID, szName, szState, lValue) VALUES(%u, 'guild_manage', 'withdraw_time', %u)", GetTablePostfix(), p->dwInfo,
...
void CClientManager::GuildDisband(CPeer* peer, TPacketGuild* p)
{
    ...
    snprintf(szQuery, sizeof(szQuery),
             "REPLACE INTO quest%s (dwPID, szName, szState, lValue) SELECT pid, 'guild_manage', 'withdraw_time', %u FROM guild_member%s WHERE guild_id = %u", GetTablePostfix(),
             (DWORD) GetCurrentTime(), GetTablePostfix(), p->dwGuild);
    CDBManager::instance().AsyncQuery(szQuery);
