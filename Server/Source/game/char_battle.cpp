...
void CHARACTER::DeathPenalty(BYTE bTown)
{
    ...
    if (GetLevel() < 10)
    {
        ...
        ChatPacket(CHAT_TYPE_INFO, "[LS;901]"/* "You did not lose any Experience because of the Blessing of the Dragon God." */);
        return;
    }
    ...
    if (IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_DEATH_PENALTY))
    {
        ...
        if (LC_IsYMIR()) // 천마 버전에서는 언제나 용신의 가호 아이템을 체크한다.
        {
            if (FindAffect(AFFECT_NO_DEATH_PENALTY))
            {
                ...
                ChatPacket(CHAT_TYPE_INFO, "[LS;901]"/* "You did not lose any Experience because of the Blessing of the Dragon king." */);
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
    ...
    if (pkKiller && pkKiller->IsPC())
    {
        ...
        {
            CGuild * g1 = GetGuild();
            if (CGuild * g2 = pkKiller->GetGuild(); g1 && g2)
        ...
        else
        {
            ...
            else
            {
                if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel)
                {
                    ...
                    {
                        pkKiller->ChatPacket(CHAT_TYPE_INFO, "[LS;910]"/* "You did not drop any Item(s) as you are protected by the Dragon God." */);
                    }
                    else
                    {
        ...
        if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
        {
            if (!(pkKiller && pkKiller->IsPC() && pkKiller->GetGuild() && pkKiller->GetGuild()->UnderAnyWar(GUILD_WAR_TYPE_FIELD)))
            {
                ...
                if (GetMobTable().dwResurrectionVnum)
                {
                    ...
                    if (LPCHARACTER chResurrect = CHARACTER_MANAGER::instance().SpawnMob(GetMobTable().dwResurrectionVnum, GetMapIndex(), GetX(), GetY(), GetZ(), true, (int) GetRotation());
                        GetDungeon() && chResurrect)
                    {
            ...
            else
            {
                if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
                {
                    ...
                    pkKiller->ChatPacket(CHAT_TYPE_INFO, "[LS;558]"/* "[Guild] There are no experience points for hunting during a guild war." */);
                }
            }
        }
    }
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type)  // returns true if dead
{
        ...
        if (pAttacker)
        {
            ...
            if (iPenetratePct)
            {
                ...
                if (number(1, 100) <= iPenetratePct)
                {
                    ...
                    if (test_server)
                    {
                        ChatPacket(CHAT_TYPE_INFO, "[LS;911;%d]"/* "Additional Stabbing Weapon Damage %d" */, GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
                    }
    ...
    else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
    {
        ...
        else if (type == DAMAGE_TYPE_NORMAL_RANGE)
        {
            ...
            if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
            {
                if (test_server)
                {
                    pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s Dodge! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
                    ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s Dodge! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
                }
        ...
        {
            dam = (dam * (100 + GetSkillPower(SKILL_JEONGWI) * 25 / 100) / 100);
        }
    ...
    if (pAttacker)
    {
        ...
        if (m_bDetailLog)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;915;%s;%d;%d;%d]"/* "%s[%d]'s Attack Position: %d %d" */, pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(),
                       pAttacker->GetY());
        }
    }
    ...
    inline void Distribute(LPCHARACTER ch, int iExp)
    {
        ...
        else if (pParty)
        {
            ...
            if (pParty->GetExpCentralizeCharacter())
            {
                ...
                {
                    auto iExpCenteralize = (int)(iExp * 0.05f);
...
struct FuncPullMonster
{
    ...
    void operator()(LPENTITY ent)
    {
        if (ent->IsType(ENTITY_CHARACTER))
        {
            ...
            auto tx = (long)(ch->GetX() + fx);
            auto ty = (long)(ch->GetY() + fy);
...
void CHARACTER::UpdateAggrPoint(LPCHARACTER pAttacker, EDamageType type, int dam)
{
    ...
    if (it == m_map_kDamage.end())
    {
        m_map_kDamage.try_emplace(pAttacker->GetVID(), 0, dam);
        it = m_map_kDamage.find(pAttacker->GetVID());
    }
