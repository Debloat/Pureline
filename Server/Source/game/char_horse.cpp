...
bool CHARACTER::StartRiding()
{
    ...
    // 턱시도 입은 상태의 말 타기 금지
    if (LPITEM armor = GetWear(WEAR_BODY); armor && (armor->GetVnum() >= 11901 && armor->GetVnum() <= 11904))
    {
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
    if (bSummon)
    {
        ...
        if (const char* pHorseName = CHorseNameManager::instance().GetHorseName(GetPlayerID()); pHorseName != nullptr && strlen(pHorseName) != 0)
        {
