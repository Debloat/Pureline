...
bool CBattleArena::IsRunning() const
{
    ...
    void operator()(LPENTITY ent)
    {
        if (ent->IsType(ENTITY_CHARACTER) == true)
        {
            ...
            if (lpChar->IsPC() == true)
            {
                if (!test_server)
                {
                    if (lpChar->GetGMLevel() != GM_PLAYER)
                    {
                        return;
                    }
                }
...
{
    auto * pInfo = dynamic_cast<SBattleArenaInfo*> (event->info);
    ...
    if (pInfo != nullptr)
    {
        switch (pInfo->state)
        {
            ...
            case 5 :
            {
                if (LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(pInfo->nMapIndex); sectree != nullptr)
                {
