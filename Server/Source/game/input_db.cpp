...
bool GetServerLocation(TAccountTable & rTab, BYTE bEmpire)
{
    ...
    for (int i = 0; i < PLAYER_PER_ACCOUNT; ++i)
    {
        ...
        if (long lIndex = 0; !CMapLocation::instance().Get(rTab.players[i].x,
                                                           rTab.players[i].y,
                                                           lIndex,
...
void CInputDB::LoginSuccess(DWORD dwHandle, const char* data)
{
    ...
    auto * pTab = (TAccountTable*) data;

    if (auto it = g_sim.find(pTab->id); g_sim.end() != it)
    {
...
void CInputDB::PlayerCreateSuccess(LPDESC d, const char* data)
{
    ...
    if (long lIndex = 0; !CMapLocation::instance().Get(pPacketDB->player.x,
                                                       pPacketDB->player.y,
                                                       lIndex,
...
void CInputDB::ChangeName(LPDESC d, const char* data)
{
    ...
    auto * p = (TPacketDGChangeName*) data;
...
void CInputDB::PlayerLoad(LPDESC d, const char* data)
{
    ...
    if (const TMapRegion * rMapRgn = SECTREE_MANAGER::instance().GetMapRegion(lPublicMapIndex))
    {
...
void CInputDB::Boot(const char* data)
{
    ...
    auto* range = (TItemIDRangeTable*) data;
    data += size * sizeof(TItemIDRangeTable);

    auto* rangespare = (TItemIDRangeTable*) data;
    data += size * sizeof(TItemIDRangeTable);
    ...
    if (WORD endCheck = decode_2bytes(data); endCheck != 0xffff)
    {
...
{
    auto* info = dynamic_cast<quest_login_event_info*> (event->info);
...
void CInputDB::QuestLoad(LPDESC d, const char* c_pData)
{
    ...
    const auto* pQuestTable = reinterpret_cast<const TQuestTable*> (c_pData + 4);
...
void CInputDB::SafeboxLoad(LPDESC d, const char* c_pData)
{
    ...
    if (LPCHARACTER ch = d->GetCharacter(); ch->GetShopOwner() || ch->GetExchange() || ch->GetMyShop() || ch->IsCubeOpen())
    {
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
    ...
    auto* p = (TSafeboxChangePasswordPacketAnswer*) c_pData;
    if (p->flag)
    {
...
void CInputDB::LoginAlready(LPDESC d, const char* c_pData)
{
    ...
    {
        auto * p = (TPacketDGLoginAlready*) c_pData;
...
void CInputDB::MapLocations(const char* c_pData)
{
    ...
    auto * pLoc = (TMapLocation*) c_pData;
...
{
    auto * p = (TPacketGuildSkillUpdate*) c_pData;
...
{
    auto * p = (TPacketGuildWar*) c_pData;
...
{
    auto* p = (TPacketGuildWarScore*) c_pData;
...
{
    auto * p = (TPacketDGGuildMember*) c_pData;
...
void CInputDB::GuildChangeMemberData(const char* c_pData)
{
    ...
    auto * p = (TPacketGuildChangeMemberData*) c_pData;
    CGuild * g = CGuildManager::instance().TouchGuild(p->guild_id);
...
{
    auto* p = (TPacketGuildLadder*) c_pData;
...
{
    auto* p = (TPacketPartySetMemberLevel*) c_pData;
    LPPARTY pParty = CPartyManager::instance().P2PCreateParty(p->dwLeaderPID);
...
{
    auto* p = (TPacketGuildSkillUsableChange*) c_pData;
...
{
    auto* p = (TPacketDGChangeEmpirePriv*) c_pData;
...
{
    auto* p = (TPacketDGChangeGuildPriv*) c_pData;
...
{
    auto* p = (TPacketDGGuildMoneyChange*) c_pData;
...
{
    auto* p = (TPacketDGGuildMoneyWithdraw*) c_pData;
...
{
    auto* p = (TPacketSetEventFlag*) c_pData;
...
void CInputDB::BillingRepair(const char* c_pData)
{
    ...
    auto * p = (TPacketBillingRepair*) c_pData;

    for (DWORD i = 0; i < dwCount; ++i, ++p)
    {
        auto * pkLD = M2_NEW CLoginData;
...
{
    auto * p = (TPacketBillingExpire*) c_pData;
...
void CInputDB::VCard(const char* c_pData)
{
    ...
    if (std::unique_ptr<SQLMsg> pmsg1(DBManager::instance().DirectQuery("UPDATE GameTime SET LimitTime=LimitTime+%d WHERE UserID='%s'", time, p->szBuyAccount));
    pmsg1->Get()->uiAffectedRows == 0 || pmsg1->Get()->uiAffectedRows == (uint32_t) -1)
    {
    ...
    std::unique_ptr<SQLMsg> pmsg2(
        DBManager::instance().DirectQuery("UPDATE vcard,GameTime SET sell_pid='%s', buy_pid='%s', buy_account='%s', sell_time=NOW(), new_time=GameTime.LimitTime WHERE vcard.id=%u AND GameTime.UserID='%s'",
                                          p->szSellCharacter, p->szBuyCharacter, p->szBuyAccount, p->dwID, p->szBuyAccount));
...
void CInputDB::ReloadAdmin(const char* c_pData)
{
    ...
    {
        tAdminInfo const& rAdminInfo = * (tAdminInfo*)c_pData;
...
{
    const auto*     c_pData = (const char*) orig;
    BYTE        bHeader;
    BYTE bLastHeader = 0;
    int         iSize;
    ...
    for (m_iBufferLeft = bytes; m_iBufferLeft > 0;)
    {
        ...
        if (const char* pRealData = (c_pData + 9); Analyze(d, bHeader, pRealData) < 0)
        {
