                    ...
                    if (pkAff)
                    {
                                                                     ...
                                                                     m_pkChr->GetPoint(POINT_STAT) != pkAff->lApplyValue)
                        {
                            ...
                            //if (pMap->IsFlagOnBase(idx))
                            {
                                ...
                                char buf[256];
                                snprintf(buf, sizeof(buf), "[LS;381;%s;%s]"/* "The guild %s's flag has been stolen by player %s." */, pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName());
                                pMap->Notice(buf);
                            }
                        }
                    }
...
void CHARACTER::__StateIdle_NPC()
{
    ...
    else
    {
        ...
        if (!IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
        {
            ...
            // 이 곳 저 곳 이동한다.
            //
            if (LPCHARACTER pkChrProtege = GetProtege())
            {
...
void CHARACTER::__StateIdle_Monster()
{
    ...
    if (LPCHARACTER pkChrProtege = GetProtege(); pkChrProtege)
    {
...
void CHARACTER::StateMove()
{
    ...
    auto x = (int)((float)(m_posDest.x - m_posStart.x) * fRate + m_posStart.x);
    auto y = (int)((float)(m_posDest.y - m_posStart.y) * fRate + m_posStart.y);
...
void CHARACTER::StateBattle()
{
    ...
    if (DWORD dwDuration = CalculateDuration(GetLimitPoint(POINT_ATT_SPEED), 2000); (dwCurTime - m_dwLastAttackTime) < dwDuration)   // 2초 마다 공격해야 한다.
    {
...
void CHARACTER::StateFlag()
{
    ...
    snprintf(buf, sizeof(buf), "[LS;327;%s;%s]"/* "%s has captured the flag of %s!" */, pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName());
    pMap->Notice(buf);
}
...
void CHARACTER::StateHorse()
{
    ...
    auto   STATE_DURATION = (DWORD)PASSES_PER_SEC(0.5);    // 상태 지속 시간
