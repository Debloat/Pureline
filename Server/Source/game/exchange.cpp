...
bool CHARACTER::ExchangeStart (LPCHARACTER victim)
{
    ...
    if (IsObserverMode())
    {
        ChatPacket (CHAT_TYPE_INFO, "[LS;893]"/* "You cannot trade while observing." */);
        return false;
    }
    ...
    {
        ChatPacket (CHAT_TYPE_INFO, "[LS;894]"/* "If the window is open, you cannot trade with others." */);
        return false;
    }

    if (victim->IsOpenSafebox() || victim->GetShopOwner() || victim->GetMyShop() || victim->IsCubeOpen())
    {
        ChatPacket (CHAT_TYPE_INFO, "[LS;895]"/* "The other person is currently busy so you cannot trade right now." */);
        return false;
    }
    ...
    {
        ChatPacket (CHAT_TYPE_INFO, "[LS;896]"/* "The other person has cancelled the trade." */);
        return false;
    }
...
bool CExchange::AddItem (TItemPos item_pos, BYTE display_pos)
{
    ...
    {
        m_pOwner->ChatPacket (CHAT_TYPE_INFO, "[LS;710]"/* "You cannot trade this item." */);
        return false;
    }
...
bool CExchange::Accept (bool bAccept)
{
    ...
    if (m_bAccept && GetCompany()->m_bAccept)
    {
        ...
        if (!Check (&iItemCount))
        {
            GetOwner()->ChatPacket (CHAT_TYPE_INFO, "[LS;897]"/* "Not enough Yang or not enough space in the inventory." */);
            victim->ChatPacket (CHAT_TYPE_INFO, "[LS;898]"/* "The other person does not have enough Yang or does not have any space left in their inventory." */);
            goto EXCHANGE_END;
        }
        ...
        if (!CheckSpace())
        {
            GetOwner()->ChatPacket (CHAT_TYPE_INFO, "[LS;899]"/* "The other person has no space left in their inventory." */);
            victim->ChatPacket (CHAT_TYPE_INFO, "[LS;1130]"/* "There isn't enough space in your inventory." */);
            goto EXCHANGE_END;
        }
        ...
        {
            victim->ChatPacket (CHAT_TYPE_INFO, "[LS;897]"/* "Not enough Yang or not enough space in the inventory." */);
            GetOwner()->ChatPacket (CHAT_TYPE_INFO, "[LS;898]"/* "The other person does not have enough Yang or does not have any space left in their inventory." */);
            goto EXCHANGE_END;
        }

        if (!GetCompany()->CheckSpace())
        {
            victim->ChatPacket (CHAT_TYPE_INFO, "[LS;899]"/* "The other person has no space left in their inventory." */);
            GetOwner()->ChatPacket (CHAT_TYPE_INFO, "[LS;1130]"/* "There isn't enough space in your inventory." */);
            goto EXCHANGE_END;
        }
        ...
        if (Done())
        {
            ...
            if (GetCompany()->Done())
            {
                ...
                GetOwner()->ChatPacket (CHAT_TYPE_INFO, "[LS;900;%s]"/* "The trade with %s has been successful." */, victim->GetName());
                victim->ChatPacket (CHAT_TYPE_INFO, "[LS;900;%s]"/* "The trade with %s has been successful." */, GetOwner()->GetName());
