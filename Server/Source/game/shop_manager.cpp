...
CShopManager::CShopManager() = default;
...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
    ...
    if (long distance = DISTANCE_APPROX(pkChr->GetX() - pkChrShopKeeper->GetX(), pkChr->GetY() - pkChrShopKeeper->GetY()); distance >= SHOP_MAX_DISTANCE)
    {
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
    ...
    if (const int64_t nTotalMoney = static_cast<int64_t> (ch->GetGold()) + static_cast<int64_t> (dwPrice); GOLD_MAX <= nTotalMoney)
    {
...
bool ConvertToShopItemTable(IN CGroupNode* pNode, OUT TShopTableEx& shopTable)
{
    ...
    {
        std::ranges::sort(shopItems, CompareShopItemName);
    }
...
bool CShopManager::ReadShopTableEx(const char* stFileName)
{
    ...
    using TMapNPCshop = std::multimap<DWORD, TShopTableEx>;
    ...
    for (int i = 0; i < pShopNPCGroup->GetRowCount(); i++)
    {
        ...
        std::ranges::transform(shopName, shopName.begin(), (int (*)(int))std::tolower);
