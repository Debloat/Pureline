...
CGuild::CGuild(TGuildCreateParameter & cp)
{
    ...
    strlcpy(m_data.grade_array[0].grade_name, "[LS;927]"/* "Leader" */, sizeof(m_data.grade_array[0].grade_name));
    ...
    {
        strlcpy(m_data.grade_array[i].grade_name, "[LS;928]"/* "Member" */, sizeof(m_data.grade_array[i].grade_name));
...
{
    DBManager::instance().Query("UPDATE guild%s SET level=%d, exp=%u, skill_point=%d WHERE id = %u", get_table_postfix(), m_data.level, m_data.exp, m_data.skill_point,
                                m_data.guild_id);
}
...
void CGuild::ChangeGradeName(BYTE grade, const char* grade_name)
{
    ...
    DBManager::instance().FuncAfterQuery(FSendChangeGrade(GetID(), grade), "UPDATE guild_grade%s SET name = '%s' where guild_id = %u and grade = %d", get_table_postfix(), text,
...
void CGuild::ChangeGradeAuth(BYTE grade, BYTE auth)
{
    ...
    DBManager::instance().FuncAfterQuery(FSendChangeGrade(GetID(), grade), "UPDATE guild_grade%s SET auth = %d where guild_id = %u and grade = %d", get_table_postfix(), auth,
                                         m_data.guild_id, grade);
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;929]"/* "[Guild] Provided Experience is larger then left Experience." */);
        return false;
    }
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
    ...
    {
        pmsg = DBManager::instance().DirectQuery("DELETE FROM guild_comment%s WHERE id = %u AND guild_id = %u AND name = '%s'", get_table_postfix(), comment_id, m_data.guild_id,
                                                 ch->GetName());
    }

    if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t) -1)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;930]"/* "[Guild] This message cannot be deleted." */);
    }
    else
    {
...
void CGuild::RefreshCommentForce(DWORD player_id)
{
    ...
    std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT id, name, content FROM guild_comment%s WHERE guild_id = %u ORDER BY notice DESC, id DESC LIMIT %d",
                                                                   get_table_postfix(), m_data.guild_id, GUILD_COMMENT_MAX_COUNT));
...
void CGuild::SkillLevelUp(DWORD dwVnum)
{
    ...
    for (LPCHARACTER vCH : m_memberOnline)
    {
        SendSkillInfoPacket(vCH);
    }
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
    ...
    if (GetSP() < iNeededSP)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;931;%d;%d]"/* "[Guild] Not enough Dragon Ghost. (%d, %d)" */, GetSP(), iNeededSP);
        return;
    }
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;932]"/* "[Guild] You cannot use the guild skills yet." */);
        return;
    }
    ...
    if (test_server)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;933;%d;%d;%d;%u]"/* "[Guild] %d skill has been applied (%d, %d) to %u." */, dwVnum, GetSP(), iNeededSP, pid);
    }
    ...
    switch (dwVnum)
    {
        case GUILD_SKILL_TELEPORT:
            ...
            else
            {
                if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
                {
                    ...
                    {
                        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;934;%d;%d]"/* "[Guild] This player is on channel %d. (Current channel: %d)" */, pcci->bChannel, g_bChannel);
                    }
                    else
                    {
                ...
                {
                    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;935]"/* "[Guild] This player is not online." */);
                }
            }
            break;
        ...
        default:
        {
            ...
            if (!UnderAnyWar())
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;937]"/* "[Guild] This guild skill can only be used during war." */);
                return;
            }
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
    ...
    if (int iRemainSP = m_data.max_power - m_data.power; iSP > iRemainSP)
    {
        iSP = iRemainSP;
        gold = iSP * 100;
    }
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;994]"/* "[Guild] Please try again later." */);
        return;
    }
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
    ...
    if (ch->GetPlayerID() != GetMasterPID())
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;939]"/* "[Guild] Only the guild leader can withdraw Yang." */);
        return;
    }
...
void CGuild::Invite(LPCHARACTER pchInviter, LPCHARACTER pchInvitee)
{
    ...
    {
        pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;940]"/* "[Guild] The player has declined the guild invitation." */);
        return;
    }
    else if (!HasGradeAuth(GetMember(pchInviter->GetPlayerID())->grade, GUILD_AUTH_ADD_MEMBER))
    {
        pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;941]"/* "[Guild] You do not have the authority to invite someone to join the guild." */);
        return;
    }
    else if (pchInvitee->GetEmpire() != pchInviter->GetEmpire())
    {
        pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;942]"/* "[Guild] You cannot invite players from another kingdom into the guild." */);
        return;
    }
    ...
    switch (errcode)
    {
        ...
        case GERR_WITHDRAWPENALTY:
            pchInviter->ChatPacket(CHAT_TYPE_INFO,
                                   "[LS;943;%d]"/* "[Guild] This player can be invited again after %d day(s)." */,
                                   quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
            return;
        case GERR_COMMISSIONPENALTY:
            pchInviter->ChatPacket(CHAT_TYPE_INFO,
                                   "[LS;944;%d]"/* "[Guild] After the rearrangement you can invite members again after %d days." */,
                                   quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
            return;
        case GERR_ALREADYJOIN:
            pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;945]"/* "[Guild] This person is already a member of another Guild." */);
            return;
        case GERR_GUILDISFULL:
            pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;946]"/* "[Guild] The maximum guild capacity has been reached." */);
            return;
        case GERR_GUILD_IS_IN_WAR :
            pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 현재 길드가 전쟁 중 입니다."));
            return;
        case GERR_INVITE_LIMIT :
            pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 현재 신규 가입 제한 상태 입니다."));
            return;
...
void CGuild::InviteAccept(LPCHARACTER pchInvitee)
{
    ...
    switch (errcode)
    {
        ...
        case GERR_COMMISSIONPENALTY:
            ...
            return;
        case GERR_ALREADYJOIN:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;945]"/* "[Guild] This person is already a member of another Guild." */);
            return;
        case GERR_GUILDISFULL:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;946]"/* "[Guild] The maximum guild capacity has been reached." */);
            return;
        case GERR_GUILD_IS_IN_WAR :
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 현재 길드가 전쟁 중 입니다."));
            return;
        case GERR_INVITE_LIMIT :
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 현재 신규 가입 제한 상태 입니다."));
            return;
