...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
    ...
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;472]"/* "[Guild] This guild name is invalid." */);
        return 0;
    }
    ...
    if (pmsg->Get()->uiNumRows > 0)
    {
        ...
        if (!(row[0] && row[0][0] == '0'))
        {
            gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;473]"/* "[Guild] This guild name is already taken." */);
            return 0;
        }
    }
    else
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;474]"/* "[Guild] You cannot found a guild." */);
        return 0;
    }
...
static void SortGuild(std::vector<CGuild*>& v)
{
    std::ranges::sort(v, [](CGuild * g1, CGuild * g2)
    {
        ...
        return (strcmp(g1->GetName(), g2->GetName()) > 0);
    }
                     );
}
...
void CGuildManager::RequestWarOver(DWORD dwGuild1, DWORD dwGuild2, DWORD dwGuildWinner, long lReward)
{
    ...
    if (CGuild * g2 = TouchGuild(dwGuild2); g1->GetGuildWarState(g2->GetID()) != GUILD_WAR_ON_WAR)
    {
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
    ...
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "[LS;475;%s;%s]"/* "The guild %s has declared war on %s!" */, TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
        SendNotice(buf);
    }
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
    ...
    if (g1 && g2)
    {
        ...
        {
            g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, "[LS;476;%s]"/* "[Guild] %s has refused the Guild war." */, g1->GetName());
        }
    }
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
    ...
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "[LS;477;%s;%s]"/* "The guild war between %s and %s will start in a few seconds!" */, g1->GetName(), g2->GetName());
        SendNotice(buf);
    }
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "[LS;478;%s;%s]"/* "The guild war between %s and %s has begun!" */, g1->GetName(), g2->GetName());
    SendNotice(buf);
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
    if (g1 && g2)
    {
        ...
        if (bDraw)
        {
            snprintf(buf, sizeof(buf), "[LS;479;%s;%s]"/* "The guild war between %s and %s has ended in a draw." */, g1->GetName(), g2->GetName());
        }
        else
        {
            if (g1->GetWarScoreAgainstTo(g2->GetID()) > g2->GetWarScoreAgainstTo(g1->GetID()))
            {
                snprintf(buf, sizeof(buf), "[LS;480;%s;%s]"/* "%s has won the guild war against %s." */, g1->GetName(), g2->GetName());
            }
            else
            {
                snprintf(buf, sizeof(buf), "[LS;480;%s;%s]"/* "%s has won the guild war against %s." */, g2->GetName(), g1->GetName());
            }
        }
...
void CGuildManager::WarOver(DWORD guild_id1, DWORD guild_id2, bool bDraw)
{
    ...
    if (TGuildWarContainer::iterator it = m_GuildWar.find(k); m_GuildWar.end() == it)
    {
        return;
    }
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
    ...
    if (g1)
    {
        ...
        if (master1)
        {
            master1->ChatPacket(CHAT_TYPE_INFO, "[LS;482]"/* "[Guild] The guild war has been cancelled." */);
        }
    }
    ...
    if (g2)
    {
        ...
        if (master2)
        {
            master2->ChatPacket(CHAT_TYPE_INFO, "[LS;482]"/* "[Guild] The guild war has been cancelled." */);
        }
    }
    ...
    {
        char buf[256 + 1];
        snprintf(buf, sizeof(buf), "[LS;483;%s;%s]"/* "The guild war between %s and %s has been cancelled." */, g1->GetName(), g2->GetName());
        SendNotice(buf);
    }
}
