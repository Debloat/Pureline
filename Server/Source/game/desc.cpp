...
{
    auto* info = dynamic_cast<DESC::desc_event_info*> (event->info);
...
bool DESC::HandshakeProcess(DWORD dwTime, long lDelta, bool bInfiniteRetry)
{
    ...
    if (int bias = (int)(dwCurTime - (dwTime + lDelta)); bias >= 0 && bias <= 50)
    {
        if (bInfiniteRetry)
        {
...
void DESC::FlushOutput()
{
    ...
    while (buffer_size(m_lpOutputBuffer) > 0)
    {
        ...
        if (int iSecondsPassed = now_tv.tv_sec - start_tv.tv_sec; iSecondsPassed > 10)
        {
        ...
        {
            if (auto d2 = (LPDESC) fdwatch_get_client_data(m_lpFdw, event_idx); d2 != this)
            {
                continue;
            }
