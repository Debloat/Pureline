...
namespace marriage
{
    ...
    {
        if (int point = MINMAX(50, GetMarriagePoint(), 100); point < 65)
        {
            return 0;
        }
        else if (point < 80)
    ...
    {
        auto* info = dynamic_cast<near_check_event_info*> (event->info);
    ...
    void TMarriage::RequestEndWedding() const
    {
        if (!pWeddingInfo)
        {
            return;
    ...
    {
        if (TMarriage* pkMarriageFinded = Get(dwPlayerID); pkMarriageFinded && pkMarriageFinded->is_married)
        {
            return true;
        }
    ...
    {
        if (auto it = m_MarriageByPID.find(dwPlayerID); it != m_MarriageByPID.end())
        {
            return it->second;
        }
    ...
    void CManager::Add(DWORD dwPID1, DWORD dwPID2, time_t tMarryTime, const char* szName1, const char* szName2)
    {
        ...
        auto* pMarriage = M2_NEW TMarriage(dwPID1, dwPID2, 0, tMarryTime, szName1, szName2);
    ...
    void CManager::WeddingStart(DWORD dwPID1, DWORD dwPID2)
    {
        ...
        if (TWeddingInfo * pwi = pMarriage->pWeddingInfo; !pwi)
        {
            return;
        }
