...
{
    auto* info = dynamic_cast<petsystem_event_info*> (event->info);
    if (info == nullptr)
    {
...
void CPetActor::GiveBuff()
{
    ...
    if (LPITEM item = ITEM_MANAGER::instance().FindByVID(m_dwSummonItemVID); nullptr != item)
    {
...
void CPetSystem::DeletePet(DWORD mobVnum)
{
    ...
    if (CPetActor* petActor = iter->second; nullptr == petActor)
    {
...
CPetActor* CPetSystem::GetByVnum(DWORD vnum) const
{
    ...
    if (TPetActorMap::const_iterator iter = m_petActorMap.find(vnum); m_petActorMap.end() != iter)
    {
        petActor = iter->second;
    }
