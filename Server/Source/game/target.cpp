...
{
    auto * info = dynamic_cast<TargetInfo*> (event->info);
                                  ...
                                  int iSendFlag)
{
    ...
    if (auto it = m_map_kListEvent.find(dwPID); it != m_map_kListEvent.end())
    {
        ...
        {
            LPEVENT pkEvent = * (it2++);
            auto* existInfo = dynamic_cast<TargetInfo*> (pkEvent->info);
...
TargetInfo* CTargetManager::GetTargetInfo(DWORD dwPID, int iType, int iArg1)
{
    ...
    {
        LPEVENT pkEvent = * (it2++);
        auto * info = dynamic_cast<TargetInfo*> (pkEvent->info);
