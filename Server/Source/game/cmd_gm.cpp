...
ACMD(do_warp)
{
    ...
    else
    {
        ...
        if (nullptr == tch)
        {
            if (const CCI* pkCCI = P2P_MANAGER::instance().Find(arg1); nullptr != pkCCI)
            {
                if (pkCCI->bChannel != g_bChannel)
                {
                    ch->ChatPacket(CHAT_TYPE_INFO, "Target is in %d channel (my channel %d)", pkCCI->bChannel, g_bChannel);
...
ACMD(do_state)
{
    ...
    if (LPSECTREE pSec = SECTREE_MANAGER::instance().Get(tch->GetMapIndex(), tch->GetX(), tch->GetY()))
    {
...
struct notice_map_packet_func
{
    ...
    void operator()(LPDESC d)
    {
        ...
        if (d->GetCharacter()->GetMapIndex() != m_mapIndex)
        {
            return;
        }
...
LPCHARACTER chForge;
LPCHARACTER chLib;
LPCHARACTER chTemple;
LPCHARACTER chTraining;
LPCHARACTER chTree;
LPCHARACTER chPortal;
LPCHARACTER chBall;
...
struct FuncWeaken
{
    ...
    void operator()(LPENTITY ent)
    {
        ...
        if (int iDist = DISTANCE_APPROX(pkChr->GetX() - m_pkGM->GetX(), pkChr->GetY() - m_pkGM->GetY()); !m_bAll && iDist >= 1000)   // 10미터 이상에 있는 것들은 purge 하지 않는다.
        {
            return;
        }
...
ACMD(do_vote_block_chat)
{
    ...
    if (!tch)
    {
        if (CCI * pkCCI = P2P_MANAGER::instance().Find(name))
        {
...
ACMD(do_build)
{
    ...
    switch (LOWER(*arg1))
    {
        case 'c':
        {
            ...
            if (bool isSuccess = pkLand->RequestCreateObject(dwVnum,
                                                             ch->GetMapIndex(),
                                                             map_x,
                                                             map_y,
                                                             x_rot,
                                                             y_rot,
                                                             z_rot, true); !isSuccess)
            {
                if (test_server)
                {
...
ACMD(do_flush)
{
    ...
    DWORD pid = strtoul(arg1, nullptr, 10);
