...
{
    auto pkMapSectree = M2_NEW SECTREE_MAP;

    pkMapSectree->m_setting = r_setting;

    LPSECTREE tree;

    for (int x = r_setting.iBaseX; x < r_setting.iBaseX + r_setting.iWidth; x += SECTREE_SIZE)
    {
        for (int y = r_setting.iBaseY; y < r_setting.iBaseY + r_setting.iHeight; y += SECTREE_SIZE)
        {
            tree = M2_NEW SECTREE;
...
void SECTREE_MANAGER::LoadDungeon(int iIndex, const char* c_pszFileName)
{
    ...
    while (!feof(fp))
    {
        ...
        m_map_pkArea[iIndex].try_emplace(position_name, x, y, sx, sy, dir);
...
bool SECTREE_MANAGER::LoadAttribute(LPSECTREE_MAP pkMapSectree, const char* c_pszFileName, TMapSetting & r_setting)
{
    ...
    #else
    auto* abComp = M2_NEW BYTE[maxMemSize];
    #endif
    auto * attr = M2_NEW DWORD[maxMemSize];
...
const TMapRegion* SECTREE_MANAGER::FindRegionByPartialName(const char* szMapName)
{
    ...
    while (it != m_vec_mapRegion.end())
    {
        ...
        //if (rRegion.index == lMapIndex)
        //return &rRegion;
        if (rRegion.strMapName.find(szMapName) != std::string::npos)
        {
...
int SECTREE_MANAGER::GetMapIndex(long x, long y)
{
    ...
    for (auto i = m_vec_mapRegion.begin(); i != m_vec_mapRegion.end(); ++i)
    {
...
bool SECTREE_MANAGER::GetValidLocation(long lMapIndex, long x, long y, long& r_lValidMapIndex, PIXEL_POSITION & r_pos, BYTE empire)
{
    ...
    while (it != m_vec_mapRegion.end())
    {
        ...
        {
            if (LPSECTREE tree = pkSectreeMap->Find(x, y); !tree)
            {
...
{
    if (LPSECTREE_MAP pkMapSectree = GetMap(lMapIndex); !pkMapSectree)
    {
        sys_err("Cannot find SECTREE_MAP by map index %d", lMapIndex);
...
bool SECTREE_MANAGER::SaveAttributeToImage(int lMapIndex, const char* c_pszFileName, LPSECTREE_MAP pMapSrc)
{
    ...
    auto * pdwDest = (DWORD*) image.GetBasePointer();

    int pixels = 0;

    sys_log(0, "2 %p", pdwDest);

    auto * pdwLine = M2_NEW DWORD[SECTREE_SIZE / CELL_SIZE];

    for (int y = 0; y < 4 * iMapHeight; ++y)
    {
        for (int y2 = 0; y2 < SECTREE_SIZE / CELL_SIZE; ++y2)
        {
            for (int x = 0; x < 4 * iMapWidth; ++x)
            {
                SECTREEID id;
                ...
                for (int x2 = 0; x2 < SECTREE_SIZE / CELL_SIZE; ++x2)
                {
                    DWORD dwColor;
...
{
    if (LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex); sectree != nullptr)
    {
...
{
    if (LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex); nullptr != sectree)
    {
