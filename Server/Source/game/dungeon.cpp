...
void CDungeon::SetFlag(std::string name, int value)
{
    ...
    {
        m_map_Flag.try_emplace(name, value);
    }
}
...
{
    if (!m_map_pkParty.contains(pParty))
    {
...
{
    auto* info = dynamic_cast<dungeon_id_info*> (event->info);
...
{
    if (!m_set_pkCharacter.contains(ch))
    {
...
void CDungeon::IncPartyMember(LPPARTY pParty, LPCHARACTER ch)
{
    ...
    if (TPartyMap::iterator it = m_map_pkParty.find(pParty); it != m_map_pkParty.end())
    {
        it->second++;
    }
    else
...
void CDungeon::JumpParty(LPPARTY pParty, long lFromMapIndex, int x, int y)
{
    ...
    if (LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(lFromMapIndex); !pMap)
    {
        sys_err("cannot find map by index %d", lFromMapIndex);
        return;
    }
...
{
    if (auto it = m_map_pkDungeon.find(dungeon_id); it != m_map_pkDungeon.end())
    {
        return it->second;
    }
    return nullptr;
...
{
    if (auto it = m_map_pkMapDungeon.find(lMapIndex); it != m_map_pkMapDungeon.end())
    {
        return it->second;
    }
    return nullptr;
...
LPDUNGEON CDungeonManager::Create(long lOriginalMapIndex)
{
    ...
    auto pDungeon = M2_NEW CDungeon(id, lOriginalMapIndex, lMapIndex);
    if (!pDungeon)
    {
...
CDungeonManager::~CDungeonManager() = default;
...
void CDungeon::SpawnMoveUnique(const char* key, DWORD vnum, const char* pos_from, const char* pos_to)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        if (ch)
        {
            m_map_UniqueMob.try_emplace(std::string(key), ch);
...
LPCHARACTER CDungeon::SpawnMob(DWORD vnum, int x, int y, int dir)
{
    ...
    LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX + x * 100, pkSectreeMap->m_setting.iBaseY + y * 100, 0, false,
    dir == 0 ? -1 : (dir - 1) * 45);
...
{
    auto it = std::ranges::find(m_regen, regen);
    if (it == m_regen.end())
    {
        return false;
    }
...
void CDungeon::SpawnMoveGroup(DWORD vnum, const char* pos_from, const char* pos_to, int count)
{
    ...
    while (count--)
    {
        ...
        CHARACTER_MANAGER::instance().SpawnMoveGroup(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX + ai.sx, pkSectreeMap->m_setting.iBaseY + ai.sy,
                                                     pkSectreeMap->m_setting.iBaseX + ai.ex, pkSectreeMap->m_setting.iBaseY + ai.ey, tx, ty, NULL, true);
    }
}
...
bool CDungeon::IsUsePotion() const
{
    return m_bUsePotion;
}

bool CDungeon::IsUseRevive() const
{
    return m_bUseRevive;
}

int CDungeon::GetKillMobCount() const
{
    return m_iMobKill;
}
int CDungeon::GetKillStoneCount() const
{
    return m_iStoneKill;
}
...
int CDungeon::CountRealMonster() const
{
...
void CDungeon::ExitAllToStartPosition() const
{
    LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
...
{
    m_map_ItemGroup.try_emplace(group_name, item_group);
}
