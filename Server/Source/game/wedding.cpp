...
namespace marriage
{
    ...
    {
        auto* info = dynamic_cast<wedding_map_info*> (event->info);
    ...
    void WeddingMap::SetEnded()
    {
        ...
        Notice("[LS;889]"/* "The wedding is finishing soon." */);
        Notice("[LS;890]"/* "Will be left automatically." */);
    ...
    bool WeddingMap::IsMember(LPCHARACTER ch)
    {
        ...
        return m_set_pkChr.contains(ch);
    }
    ...
    bool WeddingMap::IsPlayingMusic() const
    {
        return m_isMusic;
    }
    ...
    WeddingManager::WeddingManager() = default;
    ...
    DWORD WeddingManager::__CreateWeddingMap(DWORD dwPID1, DWORD dwPID2)
    {
        ...
        {
            sys_err("CreateWeddingMap(pid1=%d, pid2=%d) / regen_do(fileName=%s, mapIndex=%d, basePos=(%d, %d)) FAILED", dwPID1, dwPID2, st_weddingMapRegenFileName.c_str(), dwMapIndex,
                    pkSectreeMap->m_setting.iBaseX, pkSectreeMap->m_setting.iBaseY);
        }
        else
        {
            sys_log(0, "CreateWeddingMap(pid1=%d, pid2=%d) / regen_do(fileName=%s, mapIndex=%d, basePos=(%d, %d)) ok", dwPID1, dwPID2, st_weddingMapRegenFileName.c_str(), dwMapIndex,
                    pkSectreeMap->m_setting.iBaseX, pkSectreeMap->m_setting.iBaseY);
        }
