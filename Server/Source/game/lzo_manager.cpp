...
{
    if (int ret = lzo1x_1_compress(src, srcsize, dest, puiDestSize, GetWorkMemory()); ret != LZO_E_OK)
    {
        return false;
    }
...
{
    if (int ret = lzo1x_decompress_safe(src, srcsize, dest, puiDestSize, GetWorkMemory()); ret != LZO_E_OK)
    {
        return false;
    }
