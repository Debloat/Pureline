...
namespace quest
{
    ...
    bool CQuestManager::Initialize()
    {
...

        {
            ...
            while (1)
            {
                ...
                while (li < s.size() && isspace(s[li]))
                {
                    li++;
                }
                while (ri > 0 && isspace(s[ri]))
                {
                    ri--;
                }
    ...
    bool CQuestManager::ExecuteQuestScript(PC& pc, const string& quest_name, const int state, const char* code, const int code_size, vector<AArgScript*>* pChatScripts,
                                           bool bUseCache)
    {
        // 실행공간을 생성
