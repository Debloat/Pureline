...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
    ...
    for (i = 0; i < size; ++i)
    {
        ...
        m_map_vid.try_emplace(m_vec_prototype[i].dwVnum, m_vec_prototype[i]);
        if (test_server)
        {
    ...
    if (LC_IsKorea() || LC_IsYMIR())
    {
        if (vnum == 50300 && bTryMagic)
        {
            ...
            do
            {
                ...
                if (CSkillProto * pkSk = CSkillManager::instance().Get(dwSkillVnum); !pkSk)
                {
                    continue;
                }
...
{
    if (LPCHARACTER o; (o = item->GetOwner()))
    {
        char szHint[64];
...
#endif
{
    ...
    if (std::unordered_set<LPITEM>::iterator it = m_set_pkItemForDelayedSave.find(item); it != m_set_pkItemForDelayedSave.end())
    {
        m_set_pkItemForDelayedSave.erase(it);
    }
...
{
    switch (int killer_levelStep = (killer_level - 1) / 10)
    {
        case 0:
...
static void __DropEvent_CharStone_DropItem(CHARACTER & killer, CHARACTER & victim, ITEM_MANAGER& itemMgr, std::vector<LPITEM>& vec_item)
{
    ...
    if (number(1, MaxRange) <= dropPercent)
    {
        ...
        if (int level_diff = victim_level - killer_level; level_diff >= +gs_dropEvent_charStone.level_range || level_diff <= -gs_dropEvent_charStone.level_range)
        {
            sys_log(log_level,
...
{
    if (auto it = m_map_ItemRefineFrom.find(dwVnum); it != m_map_ItemRefineFrom.end())
    {
        return it->second;
    }
    return 0;
...
{
    if (auto it = m_map_pkSpecialItemGroup.find(dwVnum); it != m_map_pkSpecialItemGroup.end())
    {
        return it->second;
    }
    return nullptr;
...
{
    if (auto it = m_map_pkSpecialAttrGroup.find(dwVnum); it != m_map_pkSpecialAttrGroup.end())
    {
        return it->second;
    }
    return nullptr;
