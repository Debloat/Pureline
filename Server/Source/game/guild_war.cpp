...
void CGuild::GuildWarPacket(DWORD dwOppGID, BYTE bWarType, BYTE bWarState)
{
    ...
    for (auto it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
    {
        ...
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "[LS;558]"/* "[Guild] There are no experience points for hunting during a guild war." */);
        }
...
void CGuild::RequestDeclareWar(DWORD dwOppGID, BYTE type)
{
    ...
    if (it == m_EnemyGuild.end())
    {
        if (!GuildWar_IsWarMap(type))
        {
            ...
            map_allow_log();
            NotifyGuildMaster("[LS;1048]"/* "[Guild] The enemy's guild leader is offline." */);
            return;
        }
    ...
    switch (it->second.state)
    {
        ...
        {
            NotifyGuildMaster("[LS;537]"/* "[Guild] This guild is already participating in a war." */);
        }
        break;
        default:
...
void CGuild::GuildWarEntryAccept(DWORD dwOppGID, LPCHARACTER ch)
{
    ...
    if (gw.state != GUILD_WAR_ON_WAR)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;891]"/* "The war is already over." */);
        return;
    }
...
void CGuild::SetLadderPoint(int point)
{
    ...
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "[LS;1001;%d]"/* "[Guild] The guild has reached %d points." */, point);
        for (auto it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
        {
            LPCHARACTER ch = (*it);
