...
namespace quest
{
    ...
    int highscore_register(lua_State* L)
    {
        ...
        auto * qi = M2_NEW THighscoreRegisterQueryInfo;
    ...
    int mob_spawn(lua_State* L)
    {
        ...
        auto mob_vnum = (DWORD)lua_tonumber(L, 1);
    ...
    {
        auto * info = dynamic_cast<confirm_timeout_event_info*> (event->info);
        ...
        //CHARACTER_MANAGER::info().FindByPID(info->dwReplyPID);
        if (LPCHARACTER chReply = nullptr; chReply)
        {
            // 시간 지나면 알아서 닫힘
        }
    ...
    void CQuestManager::GotoConfirmState(QuestState & qs)
    {
        ...
        auto dwVID = (DWORD) lua_tonumber(qs.co, -3);
        const char* szMsg = lua_tostring(qs.co, -2);
        auto iTimeout = (int) lua_tonumber(qs.co, -1);
    ...
    bool CQuestManager::RunState(QuestState & qs)
    {
        ...
        if (int ret = lua_resume(qs.co, qs.args); ret == 0)
        {
