...
{
    auto * pinfo = (TPacketCGLogin*) data;
...
{
    auto * pinfo = (TPacketCGLogin2*) data;
...
{
    auto * p = (TPacketCGChangeName*) data;
...
{
    auto * pinfo = (struct command_player_select*) data;
    const TAccountTable & c_r = d->GetAccountTable();
...
{
    auto * pinfo = (struct command_player_create*) data;
...
{
    auto * pinfo = (struct command_player_delete*) data;
    const TAccountTable & c_rAccountTable = d->GetAccountTable();
...
#pragma pack(1)
using TPacketGTLogin = struct SPacketGTLogin
{
    BYTE header;
    WORD empty;
    DWORD id;
...
{
    const auto* p = reinterpret_cast<const TPacketCGEmpire*> (c_pData);
...
int CInputLogin::GuildSymbolUpload (LPDESC d, const char* c_pData, size_t uiBytes)
{
    ...
    auto* p = (TPacketCGGuildSymbolUpload*) c_pData;
...
{
    auto * pCG = (TPacketCGMarkCRCList*) c_pData;
