...
namespace quest
{
    ...
    int dungeon_notice(lua_State* L)
    {
        ...
        if (LPDUNGEON pDungeon = q.GetCurrentDungeon())
        {
    ...
    int dungeon_get_flag_from_map_index(lua_State* L)
    {
        ...
        if (DWORD dwMapIndex = (DWORD) lua_tonumber(L, 2))
        {
    ...
    int dungeon_set_dest(lua_State* L)
    {
        ...
        if (LPDUNGEON pDungeon = q.GetCurrentDungeon(); pDungeon && pParty)
        {
    ...
    struct FPurgeArea
    {
        ...
        void operator()(LPENTITY ent)
        {
            ...
            {
                auto pChar = static_cast<LPCHARACTER> (ent);
    ...
    int dungeon_purge_area(lua_State* L)
    {
        ...
        if (LPSECTREE_MAP pSectree = SECTREE_MANAGER::instance().GetMap(mapIndex); nullptr != pSectree)
        {
    ...
    int dungeon_set_unique(lua_State* L)
    {
        ...
        auto vid = (DWORD) lua_tonumber(L, 2);
    ...
    int dungeon_spawn_mob(lua_State* L)
    {
        ...
        if (pDungeon)
        {
            auto dwVnum = (DWORD) lua_tonumber(L, 1);
            auto x = (long) lua_tonumber(L, 2);
            auto y = (long) lua_tonumber(L, 3);
    ...
    int dungeon_spawn_goto_mob(lua_State* L)
    {
        ...
        auto lFromX = (long)lua_tonumber(L, 1);
        auto lFromY = (long)lua_tonumber(L, 2);
        auto lToX   = (long)lua_tonumber(L, 3);
        auto lToY   = (long)lua_tonumber(L, 4);
    ...
    int dungeon_spawn_group(lua_State* L)
    {
        ...
        if (LPDUNGEON pDungeon = q.GetCurrentDungeon())
        {
            auto group_vnum = (DWORD)lua_tonumber(L, 1);
            ...
            auto count = (DWORD) lua_tonumber(L, 6);
    ...
    int dungeon_join(lua_State* L)
    {
        ...
        if (LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr(); ch->GetParty() && ch->GetParty()->GetLeaderPID() == ch->GetPlayerID())
        {
        ...
        void operator()(LPENTITY ent)
        {
            if (ent->IsType(ENTITY_CHARACTER))
            {
                ...
                if (ch->IsPC())
                {
                    ...
                    for (auto it = item_group->begin(); it != item_group->end(); it++)
                    {
    ...
    {
        if (DWORD dwMapIndex = (DWORD) lua_tonumber(L, 1))
        {
            LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(dwMapIndex);
            if (pDungeon)
            {
    ...
    {
        if (LPDUNGEON pDungeon = CQuestManager::instance().GetCurrentDungeon(); pDungeon != nullptr)
        {
