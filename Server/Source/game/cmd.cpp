...
ACMD(do_inputall)
{
    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;916]"/* "Please enter the Order in full length." */);
}
...
void interpret_command(LPCHARACTER ch, const char* argument, size_t len)
{
    ...
    if (ch->GetPosition() < cmd_info[icmd].minimum_position)
    {
        ...
        {
            case POS_MOUNTING:
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;917]"/* "You cannot do this whilst sitting on a Horse." */);
                break;

            case POS_DEAD:
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;918]"/* "You cannot do that while you are lying on the ground." */);
                break;

            case POS_SLEEPING:
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;919]"/* "In my Dreams? What?" */);
                break;

            case POS_RESTING:
            case POS_SITTING:
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;920]"/* "Get up first." */);
                break;

            case POS_FIGHTING:
                ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are in a life-or-death battle. Stay focused."));
                break;
    ...
    if (ch->GetGMLevel() >= GM_LOW_WIZARD)
    {
        ...
        {
            if (LC_IsEurope() == true || LC_IsCanada() == true)
            {
                char buf[1024];
