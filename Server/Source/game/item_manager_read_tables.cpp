...
bool ITEM_MANAGER::ReadCommonDropItemFile(const char* c_pszFileName)
{
    ...
    while (fgets(buf, 1024, fp))
    {
        ...
        for (int i = 0; i <= MOB_RANK_S_KNIGHT; ++i)
        {
            ...
            auto dwPct = (DWORD)(d[i].fPercent * 10000.0f);
            DWORD dwItemVnum = 0;
            ...
            g_vec_pkCommonDropItem[i].emplace_back(d[i].iLvStart, d[i].iLvEnd, dwPct, dwItemVnum);
        }
    }
...
bool ITEM_MANAGER::ReadMonsterDropItemGroup(const char* c_pszFileName)
{
    ...
    for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
    {
        ...
        else if (strType == "drop")
        {
            ...
            bool bNew = true;
            if (auto it = m_map_pkDropItemGroup.find(iMobVnum); it == m_map_pkDropItemGroup.end())
            {
            ...
            for (int k = 1; k < 256; ++k)
            {
                ...
                if (loader.GetTokenVector(buf, &pTok))
                {
                    ...
                    auto dwPct = (DWORD)(10000.0f * fPercent);
        ...
        else if (strType == "limit")
        {
            ...
            for (int k = 1; k < 256; k++)
            {
                ...
                if (loader.GetTokenVector(buf, &pTok))
                {
                    ...
                    auto dwPct = (DWORD)(10000.0f * fPct);
