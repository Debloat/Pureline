...
{
    auto * pinfo = (TPacketCGLogin3*) c_pData;
...
{
    //OpenID test code.
    auto *tempInfo1 = (TPacketCGLogin5*)c_pData;
    ...
    //인증 실패. 에러 처리
    if (int test_url_get_protocol = auth_OpenID(authKey, inet_ntoa(d->GetAddr().sin_addr), returnID); 0 != test_url_get_protocol)
    {
