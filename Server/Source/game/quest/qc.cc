...
static ostream & operator << (ostream & ostr, const Token& tok)
{
...
static bool check_syntax(const string& str, const string& module)
{
	if (int ret = luaL_loadbuffer(L, str.c_str(), str.size(), module.c_str()))
	{
		cerr << str << endl;
...
static void RegisterDefFunction(const string& fname)
{
	function_defs.insert(fname);
}

static void RegisterUsedFunction(const string& fname)
{
			...
			case ST_WHEN_BODY:
				{
					...
					while (1)
					{
						...
						{
							state_arg_script_map[current_when_name][current_state_name].emplace_back(current_when_condition, current_when_argument, os.str());
						}
