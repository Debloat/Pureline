...
{
    auto* info = dynamic_cast<dungeon_regen_event_info*> (event->info);
...
{
    auto* info = dynamic_cast<regen_event_info*> (event->info);
...
{
    LPREGEN next_regen;
    LPREGEN_EXCEPTION next_exc;

    for (LPREGEN regen = regen_list; regen; regen = next_regen)
    {
    ...
    for (LPREGEN_EXCEPTION exc = regen_exception_list; exc; exc = next_exc)
    {
        next_exc = exc->next;
...
{
    for (LPREGEN regen = regen_list; regen; regen = regen->next)
    {
        if (!regen->event)
        {
            continue;
