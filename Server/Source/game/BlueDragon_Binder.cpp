...
unsigned int BlueDragon_GetSkillFactor (const size_t cnt, ...)
{
    ...
    auto val = static_cast<int> (lua_tonumber (L, -1));
...
unsigned int BlueDragon_GetRangeFactor (const char* key, const int val)
{
    ...
    const auto cnt = static_cast<size_t> (luaL_getn (L, -1));
    ...
    for (size_t i = 1 ; i <= cnt ; ++i)
    {
        ...
        const auto min = static_cast<int> (lua_tonumber (L, -1));
        ...
        const auto max = static_cast<int> (lua_tonumber (L, -1));
        ...
        if (min <= val && val <= max)
        {
            ...
            const auto pct = static_cast<int> (lua_tonumber (L, -1));
...
unsigned int BlueDragon_GetIndexFactor (const char* container, const size_t idx, const char* key)
{
    ...
    const auto ret = static_cast<unsigned int> (lua_tonumber (L, -1));
