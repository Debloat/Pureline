...
bool CPVP::CanRevenge(DWORD dwPID) const
{
...
DWORD CPVP::GetLastFightTime() const
{
    return m_dwLastFightTime;
}

CPVPManager::CPVPManager() = default;
...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
    ...
    if ((pkPVP = Find(kPVP.m_dwCRC)))
    {
        ...
        {
            pkVictim->ChatPacket(CHAT_TYPE_INFO, "[LS;482;%s]"/* "The battle with %s has begun!" */, pkChr->GetName());
            pkChr->ChatPacket(CHAT_TYPE_INFO, "[LS;482;%s]"/* "The battle with %s has begun!" */, pkVictim->GetName());
        }
        return;
    }
    ...
    snprintf(msg, sizeof(msg), "[LS;503;%s]"/* "%s challenged you to a battle!" */, pkChr->GetName());

    pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
    pkChr->ChatPacket(CHAT_TYPE_INFO, "[LS;514;%s]"/* "You have challenged %s to a battle." */, pkVictim->GetName());
