...
bool CSpeedServerEmpireExp::Initialize (BYTE e)
{
    ...
    for (int i = 1; i < 6; i++)
    {
        wday_exp_table[i].emplace_back (18, 0, 50);
        wday_exp_table[i].emplace_back (24, 0, 100);
    }

    wday_exp_table[0].emplace_back (18, 0, 100);
    wday_exp_table[0].emplace_back (24, 0, 150);
    wday_exp_table[6].emplace_back (18, 0, 100);
    wday_exp_table[6].emplace_back (24, 0, 150);
...
bool CSpeedServerEmpireExp::WriteExpTable()
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        for (auto it = wday_exp_table[i].begin(); it != wday_exp_table[i].end(); it++)
        {
    ...
    for (auto holi_it = holiday_map.begin(); holi_it != holiday_map.end(); holi_it++)
    {
        fprintf (fp, "HOLIDAY %d.%d.%d", holi_it->first.year + 1900, holi_it->first.mon + 1, holi_it->first.day);
        for (auto it = holi_it->second.begin(); it != holi_it->second.end(); it++)
        {
            fprintf (fp, " %d:%d %d;", it->hour, it->min, it->exp);
        }
        fprintf (fp, "\n");
...
bool CSpeedServerEmpireExp::LoadExpTable()
{
    ...
    while (fgets (one_line, 256, fp))
    {
            ...
            TOKEN ("HOLIDAY")
        {
            ...
            holiday_map.try_emplace (Date (year - 1900, mon - 1, day), lst);
        }
    }
...
{
    if (std::map <Date, std::list <HME >>::iterator it = holiday_map.find (date); it != holiday_map.end())
    {
...
void CSpeedServerEmpireExp::InitHolidayExpTable (Date date)
{
    ...
    {
        std::list <HME> lst;
        holiday_map.try_emplace (date, lst);
    }
    else
    {
        it->second.clear();
...
HME CSpeedServerEmpireExp::GetCurrentExpPriv (int& duration, bool &is_change)
{
    ...
    {
        for (auto it = holi_it->second.begin();
    ...
    {
        for (auto it =  wday_exp_table[datetime->tm_wday].begin();
             it != wday_exp_table[datetime->tm_wday].end(); it++)
        {
            // 현재 시각이 시간 구간 안에 포함되면,
            if (total_sec < (it->hour * 3600 + it->min * 60))
