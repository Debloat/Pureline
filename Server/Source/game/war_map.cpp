...
{
    auto* info = dynamic_cast<war_map_info*> (event->info);
...
void CWarMap::DecMember(LPCHARACTER ch)
{
    ...
    if (!ch->IsObserverMode())
    {
        ...
        if (m_kMapInfo.bType == WAR_MAP_TYPE_FLAG)
        {
            ...
            if (pkAff)
            {
                if (BYTE idx; GetTeamIndex(pkAff->lApplyValue, idx))
                {
...
bool CWarMap::IsFlagOnBase(BYTE bIdx)
{
    ...
    if (const PIXEL_POSITION & pos = r.pkChrFlag->GetXYZ(); pos.x == m_kMapInfo.posStart[bIdx].x && pos.y == m_kMapInfo.posStart[bIdx].y)
    {
        return true;
    }
