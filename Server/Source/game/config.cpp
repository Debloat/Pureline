...
bool map_allow_find(int index)
{
    ...
    if (!s_set_map_allows.contains(index))
    {
        return false;
    }
...
void CheckClientVersion()
{
    ...
    while (it != set.end())
    {
        ...
        if (version > date)
        {
            d->GetCharacter()->ChatPacket(CHAT_TYPE_NOTICE, "[LS;875]"/* "You do not have the correct client version. Please install the normal patch." */);
