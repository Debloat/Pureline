...
CPartyManager::~CPartyManager() = default;
...
{
    if (TPartyMap::iterator it = m_map_pkParty.find(pid); it != m_map_pkParty.end())
    {
        return it->second;
    }

    auto pParty = M2_NEW CParty;
...
{
    auto* info = dynamic_cast<party_update_event_info*> (event->info);
    ...
    if (LPCHARACTER leader = CHARACTER_MANAGER::instance().FindByPID(pid); leader && leader->GetDesc())
    {
...
DWORD CParty::GetLeaderPID() const
{
...
void CParty::P2PJoin(DWORD dwPID)
{
    ...
    if (TMemberMap::iterator it = m_memberMap.find(dwPID); it == m_memberMap.end())
    {
        TMember Member;
        ...
        m_memberMap.try_emplace(dwPID, Member);
...
void CParty::RemoveBonus()
{
    for (auto it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
    {
        if (LPCHARACTER ch; (ch = it->second.pCharacter))
        {
...
LPCHARACTER CParty::GetNextOwnership(LPCHARACTER ch, long x, long y)
{
    ...
    while (size-- > 0)
    {
        if (LPCHARACTER pkMember = m_itNextOwner->second.pCharacter; pkMember && DISTANCE_APPROX(pkMember->GetX() - x, pkMember->GetY() - y) < 3000)
        {
            IncreaseOwnership();
            return pkMember;
        }
...
{
    if (TFlagMap::iterator it = m_map_iFlag.find(name); it != m_map_iFlag.end())
    {
...
void CParty::SetFlag(const std::string& name, int value)
{
    ...
    {
        m_map_iFlag.try_emplace(name, value);
    }
...
LPDUNGEON CParty::GetDungeon() const
{
    return m_pkDungeon;
}
...
int CParty::GetExpDistributionMode() const
{
