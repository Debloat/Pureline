...
void DBManager::Process()
{
    ...
    while ((pMsg = PopResult()))
    {
        ...
        {
            switch (static_cast<CQueryInfo*> (pMsg->pvUserData)->iQueryType)
            {
                ...
                {
                    auto* qi = reinterpret_cast<CFuncQueryInfo*> (pMsg->pvUserData);
                    qi->f(pMsg);
                    M2_DELETE(qi);
                }
                break;
                ...
                {
                    auto* qi = reinterpret_cast<CFuncAfterQueryInfo*> (pMsg->pvUserData);
                    qi->f();
                    M2_DELETE(qi);
                }
                break;
...
void DBManager::SetBilling(DWORD dwKey, bool bOn, bool bSkipPush)
{
    ...
    if (auto it2 = mapLDBilling.find(ld->GetLogin()); it2 != mapLDBilling.end())
        if (it2->second != ld)
        {
            DeleteLoginData(it2->second);
        }

    mapLDBilling.try_emplace(ld->GetLogin(), ld);
...
static bool GetGameTime(MYSQL_RES * pRes, BYTE & bBillType, int& seconds)
{
    if (!pRes)
    {
        return true;
    ...
    switch (qi->iType)
    {
        ...
        case QID_DB_STRING:
        {
            ...
            for (uint i = 0; i < pMsg->Get()->uiNumRows; ++i)
            {
                ...
                if (row[0] && row[1])
                {
                    m_map_dbstring.try_emplace(std::string(row[0]), row[1]);
                    sys_log(0, "DBSTR '%s' '%s'", row[0], row[1]);
                }
            }
            if (m_map_dbstring.contains("GREET"))
            {
        ...
        {
            auto * info = (THighscoreRegisterQueryInfo*) qi->pvData;
            bool bQuery = true;
...
void AccountDB::ReturnQuery(int iType, DWORD dwIdent, void* pvData, const char* c_pszFormat, ...)
{
    ...
    auto * p = M2_NEW CReturnQueryInfo;
