...
namespace quest
{
    ...
    void PC::SetFlag (const string& name, int value, bool bSkipSave)
    {
        ...
        if (TFlagMap::iterator it = m_FlagMap.find (name); it == m_FlagMap.end())
        {
            m_FlagMap.try_emplace (name, value);
        }
    ...
    void PC::SaveFlag (const string & name, int value)
    {
        ...
        {
            m_FlagSaveMap.try_emplace (name, value);
        }
        else if (it->second != value)
        {
            it->second = value;
    ...
    void PC::SetQuest (const string& quest_name, QuestState& qs)
    {
        ...
        if (QuestInfo::iterator it = m_QuestInfo.find (qi); it == m_QuestInfo.end())
        {
            m_QuestInfo.try_emplace (qi, qs);
        }
        else
        {
            it->second = qs;
    ...
    {
        RemoveTimer (name);
        m_TimerMap.try_emplace (name, pEvent);
    ...
    void PC::RemoveTimerNotCancel (const string & name)
    {
        ...
        if (TTimerMap::iterator it = m_TimerMap.find (name); it != m_TimerMap.end())
        {
    ...
    void PC::DoQuestStateChange()
    {
        ...
        for (DWORD i = 0; i < vecQuestStateChange.size(); ++i)
        {
            ...
            if (it == quest_end())
            {
                ...
                m_QuestInfo.try_emplace (dwQuestIdx, qs);
    ...
    void PC::Reward (LPCHARACTER ch)
    {
        if (m_bIsGivenReward)
        {
            ch->ChatPacket (CHAT_TYPE_INFO, "[LS;811]"/* "[Quest] You have already received your reward." */);
            m_bIsGivenReward = false;
        }

        for (auto it = m_vRewardData.begin(); it != m_vRewardData.end(); ++it)
        {
            switch (it->type)
            {
    ...
    void PC::Build()
    {
        ...
        for (it = m_FlagMap.begin(); it != m_FlagMap.end(); ++it)
        {
            if (it->first.size() > 9 && it->first.compare (it->first.size() - 9, 9, ".__status") == 0)
            {
                ...
                m_QuestInfo.try_emplace (dwQuestIndex, qs);
            }
        }
    }
