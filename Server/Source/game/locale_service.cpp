...
int check_name_latin1 (const char* str)
{
    ...
    for (const char* tmp = str; *tmp; ++tmp)
    {
        ...
        if (unsigned char uc_tmp = *tmp; uc_tmp == 145 || uc_tmp == 146 || uc_tmp == 196
...
{
    if (static const char* sjis_symbols = "?"; strncmp (src, sjis_symbols, 2) == 0)
    {
        return true;
    }
...
int check_name_sjis (const char* str)
{
    ...
    while (*p)
    {
        if (is_char_sjis (p, e))
        {
            ...
            // END_OF_DISABLE_SPECIAL_CHAR_NAMING
            // 이문자는 허용되지 않는다.
            if ((BYTE)p[0] == 0x81 && (BYTE)p[1] == 0x40)
            {
                return false;
            }
...
bool LC_IsJapan()
{
    return LC_GetLocalType() == LC_JAPAN ? true : false;
}
bool LC_IsEnglish()
{
...
bool LC_IsHongKong()
{
    return LC_GetLocalType() == LC_HONGKONG ? true : false;
}
bool LC_IsNewCIBN()
{
    return LC_GetLocalType() == LC_NEWCIBN ? true : false;
}
bool LC_IsGermany()
{
    return LC_GetLocalType() == LC_GERMANY ? true : false;
}
bool LC_IsKorea()
{
...
bool LC_IsCanada()
{
    return LC_GetLocalType() == LC_CANADA ? false : false;
}
bool LC_IsBrazil()
{
    return LC_GetLocalType() == LC_BRAZIL ? true : false;
}
bool LC_IsSingapore()
{
    return LC_GetLocalType() == LC_SINGAPORE ? true : false;
}
bool LC_IsVietnam()
{
    return LC_GetLocalType() == LC_VIETNAM ? true : false;
}
bool LC_IsThailand()
{
    return LC_GetLocalType() == LC_THAILAND ? true : false;
}
bool LC_IsWE_Korea()
{
...
bool LC_IsTaiwan()
{
    return LC_GetLocalType() == LC_TAIWAN ? true : false;
}
