...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
    ...
    if (g)
    {
        ...
        {
            char buf[100];
            snprintf(buf, sizeof(buf), "[LS;722;%s;%s;%d]"/* "%s of the Guild %s raised up to %d%% !" */, g->GetName(), GetPrivName(type), value);
            SendNotice(buf);
        }
        else
        {
            char buf[100];
            snprintf(buf, sizeof(buf), "[LS;723;%s;%s]"/* "%s of the Guild %s normal again." */, g->GetName(), GetPrivName(type));
            SendNotice(buf);
        }
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
    ...
    {
        char buf[100];
        snprintf(buf, sizeof(buf), "[LS;724;%s;%s;%d]"/* "%s: %s has increased by %d%%!" */, GetEmpireName(empire), GetPrivName(type), value);
    ...
    {
        char buf[100];
        snprintf(buf, sizeof(buf), "[LS;726;%s;%s]"/* "%s 's %s normal again." */, GetEmpireName(empire), GetPrivName(type));
...
{
    if (SPrivEmpireData* pkPrivEmpireData = GetPrivByEmpireEx(bEmpire, type))
    {
...
int CPrivManager::GetPrivByCharacter(DWORD pid, BYTE type)
{
    ...
    if (auto it = m_aPrivChar[type].find(pid); it != m_aPrivChar[type].end())
    {
        return it->second;
    }
