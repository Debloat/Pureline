...
class CDungeon
{
    ...
    public:
        ...
        {
            m_iMonsterCount++;
            sys_log(0, "MonsterCount %d", m_iMonsterCount);
        }
        void    DecMonster()
        {
            m_iMonsterCount--;
        ...
        int CountMonster()
        {
            return m_iMonsterCount;    // 데이터로 리젠한 몬스터의 수
        }
        int CountRealMonster() const;             // 실제로 맵상에 있는 몬스터
        ...
        int GetKillMobCount() const;
        int GetKillStoneCount() const;
        bool    IsUsePotion() const;
        bool    IsUseRevive() const;
        ...
        void    ExitAll() const;
        void    ExitAllToStartPosition() const;
