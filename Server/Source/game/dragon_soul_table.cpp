...
bool DragonSoulTable::GetWeight (BYTE ds_type, BYTE grade_idx, BYTE step_index, BYTE strength_idx, OUT float& fWeight)
{
    ...
    if (nullptr != pDragonSoulGroup)
    {
        if (!pDragonSoulGroup->GetGroupValue (g_astGradeName[grade_idx], g_astStepName[step_index], strength_idx, fWeight))
        {
            sys_err ("Invalid float. DragonSoulGroup(%s) Grade(%s) Row(%s) Col(%d))", stDragonSoulName.c_str(), g_astGradeName[grade_idx].c_str(), g_astStepName[step_index].c_str(),
                     strength_idx);
            return false;
        }
        else
    ...
    sys_err ("Invalid value. DragonSoulGroup(%s) Grade(%s) Row(%s) Col(%d))", stDragonSoulName.c_str(), g_astGradeName[grade_idx].c_str(), g_astStepName[step_index].c_str(),
             strength_idx);
    return false;
}
...
bool DragonSoulTable::GetRefineStrengthValues (BYTE ds_type, BYTE material_type, BYTE strength_idx, OUT int& fee, OUT float& prob)
{
    ...
    if (std::string stStrengthIdx = boost::lexical_cast <std::string> ((int)strength_idx);
        !m_pRefineStrengthTableNode->GetGroupValue (stDragonSoulName, g_astMaterialName[material_type], stStrengthIdx, prob))
    {
