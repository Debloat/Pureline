...
class DebugAllocatorAdapter
{
    public:
        ...
        static size_t MarkAcquired(void* p, const char* file, size_t line, const char* context)
        {
            ...
            size_t age = 0;
            if (AllocMapType::iterator it = alloc_map.find(p); it == alloc_map.end())
            {
                AllocTag tag(file, line);
                alloc_map.try_emplace(p, tag);
                age = tag.age;
            }
            else
            {
    ...
    private:
                             ...
                             bool verify_age = false, size_t age = 0)
        {
            ...
            if (AllocMapType::iterator it = alloc_map_.find(p); it != alloc_map_.end())
            {
                AllocTag& tag = it->second;
                if (tag.in_use)
                {
        ...
        void DumpLeakReport()
        {
            auto it = alloc_map_.begin();
            auto end = alloc_map_.end();
            for (; it != end; ++it)
            {
                if (it->second.in_use)
                {
        ...
        using AllocMapType = std::unordered_map<void *, AllocTag>;
