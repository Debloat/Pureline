...
void CHARACTER::OpenMyShop(const char* c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
    ...
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;1025]"/* "You can only open the shop if you take off your armour." */);
        return;
    }
    ...
    if (GOLD_MAX <= nTotalMoney)
    {
        ...
        ChatPacket(CHAT_TYPE_INFO, "[LS;811]"/* "You cannot trade because you are carrying more than 2 billion Yang." */);
        return;
    }
    ...
    if (LC_IsCanada() == false)
    {
        ...
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;812]"/* "You can't give your shop an invalid name." */);
            return;
        }
    }
    ...
    for (BYTE i = 0; i < bItemCount; ++i)
    {
        ...
        if (LPITEM pkItem = GetItem((pTable + i)->pos))
        {
            if (const TItemTable * item_table = pkItem->GetProto(); item_table && (IS_SET(item_table->dwAntiFlags, ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_MYSHOP)))
            {
                ChatPacket(CHAT_TYPE_INFO, "[LS;813]"/* "You can't sell Item-Shop items in a private shop." */);
                return;
            }
...
void CHARACTER::RestartAtSamePos()
{
    ...
    while (it != m_map_view.end())
    {
        ...
        {
            auto lpChar = (LPCHARACTER)entity;
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
    ...
    if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
    {
        ...
        if (IsPC() == true && (LC_IsEurope() == true || LC_IsCanada() == true))
        {
            ...
            for (auto iter = m_map_view.begin(); iter != m_map_view.end(); iter++)
            {
                ...
                if (pEntity != nullptr)
                {
                    if (pEntity->IsType(ENTITY_CHARACTER) == true)
                    {
                        ...
                        {
                            auto pChar = (LPCHARACTER)pEntity;
...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
    ...
    if (DWORD dwPlayedTime = (get_dword_time() - m_dwPlayStartTime); dwPlayedTime > 60000)
    {
...
void CHARACTER::SaveReal()
{
    ...
    if (quest::PC * pkQuestPC = quest::CQuestManager::instance().GetPCForce(GetPlayerID()); !pkQuestPC)
    {
...
using BGMInfoMap = std::map<unsigned int, BGMInfo>;
...
{
    return m_pkMobData ? IS_SET(m_pkMobData->m_table.dwRaceFlag, dwBit) : false;
}
...
void CHARACTER::ComputePoints()
{
    ...
    RefreshAffect();
    if (CPetSystem* pPetSystem = GetPetSystem(); nullptr != pPetSystem)
    {
...
DWORD CHARACTER::GetMotionMode() const
{
    ...
    if (LPITEM pkItem; (pkItem = GetWear(WEAR_WEAPON)))
    {
...
void CHARACTER::PointChange(BYTE type, int amount, bool bAmount, bool bBroadcast)
{
    ...
    switch (type)
    {
        ...
        case POINT_GOLD:
        {
            if (const int64_t nTotalMoney = static_cast<int64_t> (GetGold()) + static_cast<int64_t> (amount); GOLD_MAX <= nTotalMoney)
            {
...
void CHARACTER::MonsterLog(const char* format, ...)
{
    ...
    if (int len2 = vsnprintf(chatbuf + len, sizeof(chatbuf) - len, format, args); len2 < 0 || len2 >= (int) sizeof(chatbuf) - len)
    {
...
void CHARACTER::mining_cancel()
{
    if (m_pkMiningEvent)
    {
        ...
        event_cancel(&m_pkMiningEvent);
        ChatPacket(CHAT_TYPE_INFO, "[LS;815]"/* "Mining is finished." */);
    }
}
...
void CHARACTER::mining(LPCHARACTER chLoad)
{
    ...
    if (LPITEM pick = GetWear(WEAR_WEAPON); !pick || pick->GetType() != ITEM_PICK)
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;816]"/* "You need a Pickaxe in order to extract ore!" */);
        return;
    }
...
void CHARACTER::fishing()
{
    ...
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;817]"/* "Please choose a Fishing Pole." */);
        return;
    }

    if (0 == rod->GetSocket(2))
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;818]"/* "Place the Bait on the Hook first." */);
        return;
    }
...
{
    if (LPITEM rod = GetWear(WEAR_WEAPON); rod && rod->GetType() == ITEM_ROD)
    {
        using fishing::fishing_event_info;
        if (m_pkFishingEvent)
        {
            auto* info = dynamic_cast<struct fishing_event_info*> (m_pkFishingEvent->info);
    ...
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;819]"/* "You can't go fishing without a fishing pole!" */);
    }
...
{
    auto * info = dynamic_cast<TPartyJoinEventInfo*> (event->info);
    ...
    if (LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(info->dwGuestPID))
    {
...
bool CHARACTER::RequestToParty(LPCHARACTER leader)
{
    ...
    if (!leader)
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;820]"/* "[Group] You cannot make a request because the group leader is not online." */);
        return false;
    }
    ...
    switch (PartyJoinErrCode errcode = IsPartyJoinableCondition(leader, this))
    {
        case PERR_NONE:
            break;

        case PERR_SERVER:
            ChatPacket(CHAT_TYPE_INFO, "[LS;530]"/* "[Group] The server cannot execute this group request." */);
            return false;

        case PERR_DIFFEMPIRE:
            ChatPacket(CHAT_TYPE_INFO, "[LS;821]"/* "[Group] You cannot form a group with players from another kingdom." */);
            return false;

        case PERR_DUNGEON:
            ChatPacket(CHAT_TYPE_INFO, "[LS;822]"/* "[Group] You cannot invite players while you are in a dungeon." */);
            return false;

        case PERR_OBSERVER:
            ChatPacket(CHAT_TYPE_INFO, "[LS;823]"/* "[Group] You cannot invite a player while you are in observer mode." */);
            return false;

        case PERR_LVBOUNDARY:
            ChatPacket(CHAT_TYPE_INFO, "[LS;824]"/* "[Group] Only players with a level difference of -30 to +30 can be invited." */);
            return false;

        case PERR_LOWLEVEL:
            ChatPacket(CHAT_TYPE_INFO, "[LS;826]"/* "[Group] You cannot invite this player, as their level is too low." */);
            return false;

        case PERR_HILEVEL:
            ChatPacket(CHAT_TYPE_INFO, "[LS;827]"/* "[Group] You cannot invite this player, as their level is too high." */);
            return false;
        ...
        case PERR_PARTYISFULL:
            ChatPacket(CHAT_TYPE_INFO, "[LS;828]"/* "[Group] You cannot invite any more players into your group." */);
            return false;
    ...
    ChatPacket(CHAT_TYPE_INFO, "[LS;829;%s]"/* "You want to join %s's group." */, leader->GetName());
    return true;
}
...
void CHARACTER::DenyToParty(LPCHARACTER member)
{
    ...
    auto * info = dynamic_cast<TPartyJoinEventInfo*> (member->m_pkPartyRequestEvent->info);
...
void CHARACTER::AcceptToParty(LPCHARACTER member)
{
    ...
    if (!GetParty())
    {
        member->ChatPacket(CHAT_TYPE_INFO, "[LS;830]"/* "This player is not in this group." */);
    }
    else
    {
        ...
        switch (errcode)
        {
            case PERR_NONE:
                ...
                return;
            case PERR_SERVER:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;530]"/* "[Group] The server cannot execute this group request." */);
                break;
            case PERR_DUNGEON:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;822]"/* "[Group] You cannot invite players while you are in a dungeon." */);
                break;
            case PERR_OBSERVER:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;823]"/* "[Group] You cannot invite a player while you are in observer mode." */);
                break;
            case PERR_LVBOUNDARY:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;824]"/* "[Group] Only players with a level difference of -30 to +30 can be invited." */);
                break;
            case PERR_LOWLEVEL:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;826]"/* "[Group] You cannot invite this player, as their level is too low." */);
                break;
            case PERR_HILEVEL:
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;827]"/* "[Group] You cannot invite this player, as their level is too high." */);
                break;
            case PERR_ALREADYJOIN:
                break;
            case PERR_PARTYISFULL:
            {
                ChatPacket(CHAT_TYPE_INFO, "[LS;828]"/* "[Group] You cannot invite any more players into your group." */);
                member->ChatPacket(CHAT_TYPE_INFO, "[LS;831]"/* "[Group] You cannot join this group because it is already full." */);
                break;
            }
            default:
                sys_err("Do not process party join error(%d)", errcode);
...
{
    auto * pInfo = dynamic_cast<TPartyJoinEventInfo*> (event->info);
...
void CHARACTER::PartyInvite(LPCHARACTER pchInvitee)
{
    ...
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;832]"/* "[Group] You do not have the right to invite someone." */);
        return;
    }
    else if (pchInvitee->IsBlockMode(BLOCK_PARTY_INVITE))
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;833;%s]"/* "[Group] %s has declined your group invitation." */, pchInvitee->GetName());
        return;
    }
    ...
    switch (PartyJoinErrCode errcode = IsPartyJoinableCondition(this, pchInvitee))
    {
        ...
        case PERR_ALREADYJOIN:
            ChatPacket(CHAT_TYPE_INFO, "[LS;834;%s]"/* "[Group] %s is already in the group." */, pchInvitee->GetName());
            return;
...
void CHARACTER::PartyInviteAccept(LPCHARACTER pchInvitee)
{
    ...
    switch (PartyJoinErrCode errcode = IsPartyJoinableMutableCondition(this, pchInvitee))
    {
        case PERR_NONE:
            break;

        case PERR_SERVER:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;530]"/* "[Group] The server cannot execute this group request." */);
            return;

        case PERR_DUNGEON:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;835]"/* "[Group] You cannot accept an invitation into a dungeon." */);
            return;

        case PERR_OBSERVER:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;823]"/* "[Group] You cannot invite a player while you are in observer mode." */);
            return;

        case PERR_LVBOUNDARY:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;824]"/* "[Group] Only players with a level difference of -30 to +30 can be invited." */);
            return;

        case PERR_LOWLEVEL:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;826]"/* "[Group] You cannot invite this player, as their level is too low." */);
            return;

        case PERR_HILEVEL:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;827]"/* "[Group] You cannot invite this player, as their level is too high." */);
            return;

        case PERR_ALREADYJOIN:
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;837]"/* "[Group] You cannot accept the invitation." */);
            return;

        case PERR_PARTYISFULL:
            ChatPacket(CHAT_TYPE_INFO, "[LS;828]"/* "[Group] You cannot invite any more players into your group." */);
            pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;831]"/* "[Group] You cannot join this group because it is already full." */);
            return;
...
void CHARACTER::PartyInviteDeny(DWORD dwPID)
{
    ...
    if (pchInvitee)
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;838;%s]"/* "[Group] %s has declined your invitation." */, pchInvitee->GetName());
    }
}

void CHARACTER::PartyJoin(LPCHARACTER pLeader)
{
    pLeader->ChatPacket(CHAT_TYPE_INFO, "[LS;839;%s]"/* "[Group] %s has joined your group." */, GetName());
    ChatPacket(CHAT_TYPE_INFO, "[LS;840;%s]"/* "[Group] %s has joined your Group." */, pLeader->GetName());
        ...
        if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
        {
            ...
            if (GetMyShop())
            {
                ...
                if (pkChrCauser == this) // 자기는 가능
                {
                    ...
                    {
                        pkChrCauser->ChatPacket(CHAT_TYPE_INFO, "[LS;841]"/* "You can't use a private shop now." */);
                        return;
                    }
                }
                else // 다른 사람이 클릭했을때
                {
                    ...
                    {
                        pkChrCauser->ChatPacket(CHAT_TYPE_INFO, "[LS;842]"/* "This player is already trading with another player." */);
                        return;
                    }
                }
                //END_PREVENT_TRADE_WINDOW
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
    ...
    false == IsPet())
    {
        ...
        if (fFollowSpeed >= 0.1f)
        {
            ...
            float fYourMoveEstimateX;
            float fYourMoveEstimateY;
...
void CHARACTER::ReqSafeboxLoad(const char* pszPassword)
{
    ...
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;526]"/* "[Storeroom] You have entered an incorrect password." */);
        return;
    }
    else if (m_pkSafebox)
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;527]"/* "[Storeroom] The Storeroom is already open." */);
        return;
    }

    if (int iPulse = thecore_pulse(); iPulse - GetSafeboxLoadTime()  < PASSES_PER_SEC(10))
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;528]"/* "[Storeroom] You have to wait 10 seconds before you can open the Storeroom again." */);
        return;
    }
    else if (GetDistanceFromSafeboxOpen() > 1000)
    {
        ChatPacket(CHAT_TYPE_INFO, "[LS;529]"/* "[Storeroom] You are too far away to open the storeroom." */);
        return;
    }
...
bool CHARACTER::BuildUpdatePartyPacket(TPacketGCPartyUpdate & out)
{
    ...
    if (LPCHARACTER l = GetParty()->GetLeaderCharacter(); l && DISTANCE_APPROX(GetX() - l->GetX(), GetY() - l->GetY()) < PARTY_DEFAULT_RANGE)
    {
        if (g_iUseLocale)
        {
...
void CHARACTER::MonsterChat(BYTE bMonsterChatType)
{
    ...
    else
    {
        ...
        snprintf(sbuf, sizeof(sbuf), "(locale.monster_chat[%i] and locale.monster_chat[%i][number(1, table.getn(locale.monster_chat[%i]))] or '')", GetRaceNum(), GetRaceNum(),
                 GetRaceNum());
    }
    ...
    class FuncCheckWarp
    {
        public:
            ...
            void operator()(LPENTITY ent)
            {
                ...
                if (int iDist = DISTANCE_APPROX(pkChr->GetX() - m_lX, pkChr->GetY() - m_lY); iDist > 300)
                {
                    return;
                }
...
{
    if (LPCHARACTER victim; (victim = (CHARACTER_MANAGER::instance().FindByPID(dwPID))))
    {
        ...
        if (IS_SUMMONABLE_ZONE(mapIdx))
        {
            ...
            {
                ChatPacket(CHAT_TYPE_INFO, "[LS;846]"/* "You cannot teleport to the player." */);
                return false;
            }
        }
        else
    ...
    else
    {
        ...
        if (!pcci)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;848]"/* "This player is not online." */);
            return false;
        }

        if (pcci->bChannel != g_bChannel)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;849;%d;%d]"/* "The player is playing on channel %d. (You are on channel %d.)" */, pcci->bChannel, g_bChannel);
            return false;
        }
...
int CHARACTER::ComputeRefineFee(int iCost, int iMultiply) const
{
    ...
    if (pGuild)
    {
        ...
        if (LPCHARACTER chRefineNPC = CHARACTER_MANAGER::instance().Find(m_dwRefineNPCVID); chRefineNPC && chRefineNPC->GetEmpire() != GetEmpire())
        {
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
    ...
    if (iPulse - GetSafeboxLoadTime() < PASSES_PER_SEC(limittime))
    {
        if (bSendMsg)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;850;%d]"/* "After opening the Storeroom, you cannot go anywhere else for %d seconds." */, limittime);
        }
    ...
    if (bCheckShopOwner)
    {
        if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
        {
            if (bSendMsg)
            {
                ChatPacket(CHAT_TYPE_INFO, "[LS;851]"/* "You cannot go elsewhere for a period of time after the trade." */);
            }
    ...
    if (iPulse - GetExchangeTime()  < PASSES_PER_SEC(limittime))
    {
        if (bSendMsg)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;852;%d]"/* "After trading you cannot go elsewhere for %d seconds." */, limittime);
        }
        return true;
    }
    //END_PREVENT_PORTAL_AFTER_EXCHANGE
    ...
    if (iPulse - GetRefineTime() < PASSES_PER_SEC(limittime))
    {
        if (bSendMsg)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;437;%d]"/* "After a trade, you cannot use a scroll for another %d seconds." */, limittime);
        }
        return true;
    }
...
{
    if (LPPARTY party = GetParty(); party != nullptr)
    {
