...
bool CLand::RequestCreateObject(DWORD dwVnum, long lMapIndex, long x, long y, float xRot, float yRot, float zRot, bool checkAnother)
{
    ...
    auto tsx = (int)(pkProto->lRegion[0] * cosf(rad) + pkProto->lRegion[1] * sinf(rad) + x);
    auto tsy = (int)(pkProto->lRegion[0] * -sinf(rad) + pkProto->lRegion[1] * cosf(rad) + y);

    auto tex = (int)(pkProto->lRegion[2] * cosf(rad) + pkProto->lRegion[3] * sinf(rad) + x);
    auto tey = (int)(pkProto->lRegion[2] * -sinf(rad) + pkProto->lRegion[3] * cosf(rad) + y);
...
bool CManager::LoadObject(TObject * pTable, bool isBoot)  // from DB
{
    ...
    auto pkObj = M2_NEW CObject(pTable, pkProto);
...
void CManager::SendLandList(LPDESC d, long lMapIndex)
{
    ...
    while (it != m_map_pkLand.end())
    {
        ...
        //
        if (LPCHARACTER ch  = d->GetCharacter())
        {
