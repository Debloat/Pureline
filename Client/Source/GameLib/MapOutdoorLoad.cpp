...
bool CMapOutdoor::Load(float x, float y, float z)
{
    ...
    if (std::string strFileName = GetMapDataDirectory() + "\\Setting.txt"; !LoadSetting(strFileName.c_str()))
    {
    ...
    if (std::string local_envDataName = GetMapDataDirectory() + "\\" + m_settings_envDataName; rkPackMgr.isExist(local_envDataName.c_str()))
    {
...
bool CMapOutdoor::LoadSetting(const char* c_szFileName)
{
    ...
    if (TTokenVector & rkVec_strToken = stTokenVectorMap["textureset"]; !rkVec_strToken.empty())
    {
    ...
    D3DXMatrixScaling(&m_matDynamicShadowScale, 1.0f / 7650.0f, -1.0f / 7650.0f, 1.0f);
