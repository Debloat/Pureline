...
class CMapBase : public CScreen
{
    ...
    public:
        ...
        virtual void    ApplyLight (const D3DLIGHT9& c_rkLight) = 0;
    ...
    public:
        ...
        void SetName (const std::string & cr_strName)
        {
            m_strName = cr_strName;
        }

        bool IsCopiedMap() const
        {
            return m_strParentMapName.length() > 0;    // "원본 데이터는 따로 있는" 복사 맵인가?
        }
        const std::string& GetParentMapName() const
        {
            return m_strParentMapName;    // 원본 데이터를 갖고있는 맵의 이름을 리턴 (복사맵의 경우 필수로 세팅되어야 함)
        }
    ...
    protected:
        ...
        std::string
        m_strParentMapName;     // 원본 맵의 이름. 이 값이 세팅되어 있다면 실제 모든 맵 데이터는 ParentMap/* 경로에서 읽어온다. 맵의 일부만 읽어오는 기능은 필요없대서 패스.
        bool                    m_bReady;
