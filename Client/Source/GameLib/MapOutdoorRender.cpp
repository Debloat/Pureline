...
{
    if (DWORD dwMask = (1 << ePart); dwMask & m_dwVisiblePartFlags)
    {
        return true;
    }
...
void CMapOutdoor::RenderTree(unsigned long flags)
{
    ...
    {
        CSpeedTreeForestDirectX9::Instance().Render(flags);
    }
}
/* ----------------------------------------------------- */
...
struct FRenderPCBlocker
{
    ...
    {
        pInstance->Show();
        if (auto* pThingInstance = dynamic_cast <CGraphicThingInstance*> (pInstance); pThingInstance != nullptr)
        {
...
void CMapOutdoor::RenderArea(bool bRenderAmbience)
{
    ...
    if (bool m_isDisableSortRendering = false)
    {
        for (int i = 0; i < AROUND_AREA_NUM; ++i)
        {
            CArea * pArea;
...
void CMapOutdoor::DrawWireFrame(long patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
    ...
    if (long sPatchNum = pTerrainPatchProxy->GetPatchNum(); sPatchNum < 0)
    {
        return;
    }
    if (BYTE ucTerrainNum = pTerrainPatchProxy->GetTerrainNum(); 0xFF == ucTerrainNum)
    {
        return;
    }
...
void CMapOutdoor::RenderMarkedArea()
{
    ...
    D3DXMATRIX matTexTransformTemp;
