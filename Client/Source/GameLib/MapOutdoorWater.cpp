...
void CMapOutdoor::RenderWater()
{
    ...
    s_fWaterHeightCurrent = s_fWaterHeightBegin + (s_fWaterHeightEnd - s_fWaterHeightBegin) * (float)((CTimer::Instance().GetCurrentMillisecond() - s_dwLastHeightChangeTime) /
                                                                                                      (float) s_dwBlendtime);
...
void CMapOutdoor::DrawWater(long patchnum)
{
    ...
    /* - SHADERS ------------------------------------------- */
    if (IShader* shader = CShaderManager::Instance().GetShader(IShader::EType::WATER); shader && shader->IsVisible())
    {
