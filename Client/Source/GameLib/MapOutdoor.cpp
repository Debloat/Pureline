...
struct FGetPickingPoint
{
    ...
    void operator()(CGraphicObjectInstance * pInstance)
    {
        ...
        {
            auto * pThing = (CGraphicThingInstance*)pInstance;
...
{
    auto * pAlphaFogImage = (CGraphicImage*) CResourceManager::Instance().GetResourcePointer("D:/ymir work/special/fog.tga");
    auto * pAttrImage = (CGraphicImage*)CResourceManager::Instance().GetResourcePointer("d:/ymir work/special/white.dds");
    auto * pBuildTransparentImage = (CGraphicImage*)CResourceManager::Instance().GetResourcePointer("d:/ymir Work/special/PCBlockerAlpha.dds");
...
bool CMapOutdoor::IsWireframe() const
{
...
bool CMapOutdoor::__PickTerrainHeight(float& fPos, const D3DXVECTOR3& v3Start, const D3DXVECTOR3& v3End, float fStep, float fRayRange, float fLimitRange,
                                      D3DXVECTOR3* pv3Pick)
{
    CTerrain * pTerrain;
...
float CMapOutdoor::GetCacheHeight(float fx, float fy)
{
    ...
    if (m_kHeightCache.m_isUpdated && nx < 16 * 30000 && ny < 16 * 30000)
    {
        ...
        for (auto i = pkVct_kItem->begin(); i != pkVct_kItem->end(); ++i)
        {
...
// End of for grass
//////////////////////////////////////////////////////////////////////////
BOOL CMapOutdoor::GetAreaPointer(const BYTE c_byAreaNum, CArea** ppArea) const
{
...
BOOL CMapOutdoor::GetTerrainPointer(const BYTE c_byTerrainNum, CTerrain** ppTerrain) const
{
...
//////////////////////////////////////////////////////////////////////////
void CMapOutdoor::GetBaseXY(DWORD * pdwBaseX, DWORD * pdwBaseY) const
{
    *pdwBaseX = m_dwBaseX;
    *pdwBaseY = m_dwBaseY;
}
...
void CMapOutdoor::SpecialEffect_Create(DWORD dwID, float x, float y, float z, const char* c_szEffName)
{
    ...
    if (TSpecialEffectMap::iterator itor = m_kMap_dwID_iEffectID.find(dwID); m_kMap_dwID_iEffectID.end() != itor)
    {
    ...
    for (; itorRect != m_rkList_kGuildArea.end(); ++itorRect)
    {
        ...
        for (int ixCell = ix1Cell; ixCell <= ix2Cell; ++ixCell)
            for (int iyCell = iy1Cell; iyCell <= iy2Cell; ++iyCell)
            {
                ...
                if (CTerrain * pTerrain; !GetTerrainPointer(byTerrainNum, &pTerrain))
                {
                    continue;
                }
                ...
                {
                    auto * pbyBuf = new BYTE[CTerrain::ATTRMAP_XSIZE * CTerrain::ATTRMAP_YSIZE];
