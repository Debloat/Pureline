...
bool NRaceData::THitData::Load (CTextFileLoader & rTextFileLoader)
{
    ...
    if (CTokenVector * tv; rTextFileLoader.GetTokenVector ("hitposition", &tv))
    {
        auto it = tv->begin();
...
BOOL NRaceData::LoadMotionAttackData (CTextFileLoader & rTextFileLoader, TMotionAttackData * pData)
{
    ...
    if (DWORD dwHitDataCount; !rTextFileLoader.GetTokenDoubleWord ("hitdatacount", &dwHitDataCount))
    {
        pData->HitDataContainer.emplace_back();
    ...
    else
    {
        for (DWORD i = 0; i < dwHitDataCount; ++i)
        {
            ...
            pData->HitDataContainer.emplace_back();
            if (THitData & rHitData = * (pData->HitDataContainer.rbegin()); !rHitData.Load (rTextFileLoader))
            {
                return FALSE;
            }
...
void NRaceData::SaveMotionAttackData (FILE * File, int iTabCount, const TMotionAttackData & c_rData)
{
    ...
    for (; itor != c_rData.HitDataContainer.end(); ++itor, ++dwHitDataNumber)
    {
        ...
        for (auto it = c_rHitData.mapHitPosition.begin(); it != c_rHitData.mapHitPosition.end(); ++it)
        {
