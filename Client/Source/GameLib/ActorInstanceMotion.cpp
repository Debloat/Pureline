...
void CActorInstance::__MotionEventProcess(BOOL isPC)
{
    if (isAttacking())
    {
        auto dwNextFrame = DWORD (GetAttackingElapsedTime() * g_fGameFPS);
...
void CActorInstance::ReservingMotionProcess()
{
    ...
    if (float fCurrentTime = GetLocalTime(); rReservingMotionNode.fStartTime > fCurrentTime)
    {
        return;
    }

    switch (DWORD dwNextMotionIndex = GET_MOTION_INDEX(rReservingMotionNode.dwMotionKey))
    {
    ...
    switch (DWORD dwCurrentMotionIndex = GET_MOTION_INDEX(dwMotionKey))
    {
        case CRaceMotionData::NAME_STAND_UP:
        case CRaceMotionData::NAME_STAND_UP_BACK:
            if (IsDead())
...
WORD CActorInstance::__GetCurrentMotionIndex() const
{
    return GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey);
}

DWORD CActorInstance::__GetCurrentMotionKey() const
{
...
BOOL CActorInstance::isLock()
{
    ...
    switch (DWORD dwCurMotionIndex = __GetCurrentMotionIndex(); dwCurMotionIndex)
    {
...
DWORD CActorInstance::__SetMotion(const SSetMotionData& c_rkSetMotData, DWORD dwRandMotKey)
{
    ...
    if (IsDead())
    {
        if (uNextMot != CRaceMotionData::NAME_DAMAGE_FLYING && uNextMot != CRaceMotionData::NAME_DAMAGE_FLYING_BACK && uNextMot != CRaceMotionData::NAME_DEAD &&
    ...
    if (__IsMoveMotion())
    {
        if (uNextMot == CRaceMotionData::NAME_DAMAGE || uNextMot == CRaceMotionData::NAME_DAMAGE_BACK || uNextMot == CRaceMotionData::NAME_DAMAGE_FLYING ||
