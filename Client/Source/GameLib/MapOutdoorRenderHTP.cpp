...
void CMapOutdoor::__RenderTerrain_RenderHardwareTransformPatch()
{
    ...
    else
    {
        ...
        STATEMANAGER.SetSamplerState(1, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC);
        STATEMANAGER.SetSamplerState(1, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC);
        STATEMANAGER.SetSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_ANISOTROPIC);
    }
    #else
    CSpeedTreeWrapper::ms_bSelfShadowOn = true;
    ...
    if (1)
    {
        ...
        else
        {
            ...
            for (DWORD j = 1; j < pTerrain->GetNumTextures(); ++j)
            {
                ...
                auto aIterator = std::ranges::find(m_RenderedTextureNumVector, (int)j);
