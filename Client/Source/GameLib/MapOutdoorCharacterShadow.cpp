...
void CMapOutdoor::CreateCharacterShadowTexture()
{
    ...
    if (FAILED(ms_lpd3dDevice->CreateDepthStencilSurface(m_wShadowMapSize, m_wShadowMapSize, D3DFMT_D16, D3DMULTISAMPLE_NONE, 0, true, &m_lpCharacterShadowMapDepthSurface,
                                                         nullptr)))
    {
...
bool CMapOutdoor::BeginRenderCharacterShadowToTexture()
{
    ...
    D3DXVECTOR3 v3Eye(v3Target.x - std::numbers::sqrt3_v<float> * 1250.0f,
                      v3Target.y - 1250.0f,
    ...
    D3DXMatrixOrthoRH(&matLightProj, 7650.0f, 7650.0f, 1.0f, 15000.0f);

    /* - DYNAMIC_TREE_SHADOWS ------------------------------ */
    /* - DYNAMIC_OBJECT_SHADOWS ---------------------------- */
    CSpeedTreeForestDirectX9::Instance().UpdateShadowCompoundMatrix(
        matLightView,
        matLightProj
    );
...
/* - DYNAMIC_TREE_SHADOWS ------------------------------ */
/* - DYNAMIC_OBJECT_SHADOWS ---------------------------- */
void CMapOutdoor::RenderWorldDynamicShadowsToTexture()
{
    // 1. Pipeline state specific to world/object/tree shadow pass
    ...
    // 2. Objects
    RenderObjectShadowsToTexture();  // already a CMapOutdoor method

    CSpeedTreeForestDirectX9::Instance().Render(
        Forest_RenderBranches | Forest_RenderLeaves | Forest_RenderFronds | Forest_RenderToShadow
    );
