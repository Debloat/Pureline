...
void CArea::RenderCollision() const
{
    STATEMANAGER.SetTexture(0, nullptr);
    STATEMANAGER.SetTexture(1, nullptr);
...
void CArea::__SetObjectInstance_SetEffect(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
    ...
    if (CEffectData * pData; !rem.GetEffectData(pObjectInstance->dwEffectID, &pData))
    {
...
void CArea::__SetObjectInstance_SetBuilding(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
    ...
    auto * pThing = (CGraphicThing*)rkResMgr.GetResourcePointer(Data.strFileName.c_str());
    pThing->AddReference();
...

    {
        ...
        for (UINT uLODIndex = 1; uLODIndex <= 3; ++uLODIndex)
        {
            ...
            auto* pLODModelThing = (CGraphicThing*)rkResMgr.GetResourcePointer(stLODModelFileName.c_str());
            if (!pLODModelThing)
            {
                break;
            }
...
void CArea::__LoadAttribute(TObjectInstance * pObjectInstance, const char* c_szAttributeFileName)
{
    ...
    auto * pAttributeData = (CAttributeData*) CResourceManager::Instance().GetResourcePointer(c_szAttributeFileName);
    ...
    if (false == bFileExist)
    {
        ...
        std::ranges::transform(attrFileName, attrFileName.begin(), [](unsigned char c)
        {
            return std::tolower(c);
        });
        const bool bIsDungeonObject = (attrFileName.contains("/dungeon/")) || (attrFileName.contains("\\dungeon\\"));
