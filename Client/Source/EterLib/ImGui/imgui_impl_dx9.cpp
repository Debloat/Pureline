...
void ImGui_ImplDX9_RenderDrawData (ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (device->CreateVertexBuffer (bd->VertexBufferSize * sizeof (CUSTOMVERTEX), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &bd->pVB,
                                        nullptr) < 0)
        {
            return;
        }
    ...
    if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if (device->CreateIndexBuffer (bd->IndexBufferSize * sizeof (ImDrawIdx), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, sizeof (ImDrawIdx) == 2 ? D3DFMT_INDEX16 : D3DFMT_INDEX32,
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                device->DrawIndexedPrimitive (D3DPT_TRIANGLELIST, pcmd->VtxOffset + global_vtx_offset, 0, (UINT)draw_list->VtxBuffer.Size, pcmd->IdxOffset + global_idx_offset,
                                              pcmd->ElemCount / 3);
            }
        }
...
static bool ImGui_ImplDX9_CheckFormatSupport (LPDIRECT3DDEVICE9 pDevice, D3DFORMAT format)
{
    ...
    D3DUSAGE_DYNAMIC | D3DUSAGE_QUERY_FILTER | D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING, D3DRTYPE_TEXTURE, format)) == D3D_OK;
    pd3d->Release();
    return support;
}
...
void ImGui_ImplDX9_UpdateTexture (ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        HRESULT hr = bd->pd3dDevice->CreateTexture (tex->Width, tex->Height, 1, D3DUSAGE_DYNAMIC, bd->HasRgbaSupport ? D3DFMT_A8B8G8R8 : D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &dx_tex,
                                                    nullptr);
        if (hr < 0)
        {
