...
HFONT CGraphicFontTexture::GetFont (WORD codePage)
{
    ...
    if (TFontMap::iterator i = m_fontMap.find (codePage); i != m_fontMap.end())
    {
        hFont = i->second;
    }
    else
...
CGraphicFontTexture::TCharacterInfomation* CGraphicFontTexture::UpdateCharacterInfomation (TCharacterKey code)
{
    ...
    for (int nChrY = 0; nChrY < nChrHeight; ++nChrY, pdwDIBRow += nDIBWidth)
    {
        for (int nChrX = 0; nChrX < nChrWidth; ++nChrX)
        {
