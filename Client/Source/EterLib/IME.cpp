...
{
    public:
        CDisableCicero() : m_ptim(nullptr), m_bComInit(false)
        {
        }
        ~CDisableCicero()
        {
...
void CIME::EnableIME(bool bEnable)
{
    ...
    ImmAssociateContext(ms_hWnd, bEnable ? m_hOrgIMC : nullptr);
...
int CIME::GetReading(std::string & rstrText)
{
    ...
    if (ms_wstrReading.empty())
    {
        return 0;
    }
    int readingLen = WideCharToMultiByte(CP_UTF8, 0, &ms_wstrReading[0], ms_wstrReading.size(), reading, sizeof(reading), NULL, NULL);
...
void CIME::SetText(const char* szText, int len)
{
    ...
    ms_lastpos = MultiByteToWideChar(CP_UTF8, 0, begin, iter - begin, m_wText, m_wTextLen);
...
int  CIME::GetText(std::string & rstrText, bool addCodePage)
{
    ...
    len += WideCharToMultiByte(CP_UTF8, 0, m_wText, ms_curpos, text, sizeof(text) - len, NULL, NULL);
    len += WideCharToMultiByte(CP_UTF8, 0, m_wszComposition, ms_compLen, text + len, sizeof(text) - len, NULL, NULL);
    len += WideCharToMultiByte(CP_UTF8, 0, m_wText + ms_curpos, ms_lastpos - ms_curpos, text + len, sizeof(text) - len, NULL, NULL);
...
{
    if (m_exceptKey.end() == std::ranges::find(m_exceptKey, key))
    {
        return true;
    }
    else
...
void CIME::FinalizeString(bool bSend)
{
    ...
    if (!ms_bInitialized || s_bProcessing || nullptr == (himc = ImmGetContext(ms_hWnd)))
    {
        return;
    }
    s_bProcessing = true;
...
void CIME::AttributeProcess(HIMC hImc)
{
    ...
    for (start = 0; start < attributeLen; ++start)
        if (attribute[start] == ATTR_TARGET_CONVERTED || attribute[start] == ATTR_TARGET_NOTCONVERTED)
        {
            break;
        }
...
void CIME::CandidateProcess(HIMC hImc)
{
    ...
    if (dwCandidateLen > 0)
    {
        ...
        auto* lpCandidateList = (CANDIDATELIST*)(&abyCandidate[0]);
...
void CIME::ReadingProcess(HIMC hImc)
{
    ...
    if (_GetReadingString)
    {
        ...
        if (int wstrLen = _GetReadingString(hImc, 0, nullptr, (PINT)&dwErr, &bVertical, &uMaxUiLen); wstrLen == 0)
        {
            ms_wstrReading.resize(0);
        }
        else
        {
            auto* wstr = (wchar_t*)alloca(sizeof(wchar_t) * wstrLen);
...
bool CIME::IsMax(const wchar_t* wInput, int len)
{
    ...
    int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, m_wText, ms_lastpos, nullptr, 0, nullptr, nullptr);
    int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, wInput, len, nullptr, 0, nullptr, nullptr);
    ...
    else if (m_userMax != 0 && m_max != m_userMax)
    {
        ...
        int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, str.c_str(), str.length(), nullptr, 0, nullptr, nullptr);
        int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, input.c_str(), input.length(), nullptr, 0, nullptr, nullptr);
...
DWORD CIME::GetImeId(UINT uIndex)
{
    ...
    if (!((ms_hklCurrent == _CHT_HKL_NEW_PHONETIC) || (ms_hklCurrent == _CHT_HKL_NEW_CHANG_JIE) || (ms_hklCurrent == _CHT_HKL_NEW_QUICK) ||
    (ms_hklCurrent == _CHT_HKL_HK_CANTONESE) || (ms_hklCurrent == _CHS_HKL)))
    {
        ms_adwId[0] = ms_adwId[1] = 0;
        return 0;
    }

    if (!ImmGetIMEFileNameA(ms_hklCurrent, szTmp, std::size(szTmp) - 1))
    {
        ms_adwId[0] = ms_adwId[1] = 0;
        return 0;
    }
...
void CIME::CheckInputLocale()
{
    ...
    char szCodePage[8];
    int iRc = GetLocaleInfoA(MAKELCID(GETLANG(), SORT_DEFAULT), LOCALE_IDEFAULTANSICODEPAGE, szCodePage, COUNTOF(szCodePage));
    iRc;
    ms_uInputCodePage = _strtoul(szCodePage, nullptr, 0);
...
BOOL CTsfUiLessMode::CurrentInputLocaleIsIme()
{
    ...
    hr = CoCreateInstance(CLSID_TF_InputProcessorProfiles, nullptr, CLSCTX_INPROC_SERVER, __uuidof(ITfInputProcessorProfiles), (LPVOID*)&pProfiles);
    if (SUCCEEDED(hr))
    {
