...
{
    if (square_distance_between_linesegment_and_point (s.v3LastPosition, s.v3Position,
                                                       m_attribute.v3Position) < (m_attribute.fRadius + s.fRadius) * (m_attribute.fRadius + s.fRadius))
    {
...
bool CCylinderCollisionInstance::CollideCylinderVSDynamicSphere (const TCylinderData & c_rattribute, const CDynamicSphereInstance & s) const
{
    ...
    D3DXVECTOR3 oa;
    D3DXVECTOR3 ob;
    IntersectLineSegments (c_rattribute.v3Position, D3DXVECTOR3 (c_rattribute.v3Position.x, c_rattribute.v3Position.y, c_rattribute.v3Position.z + c_rattribute.fHeight),
...
D3DXVECTOR3 CCylinderCollisionInstance::OnGetCollisionMovementAdjust (const CDynamicSphereInstance & s) const
{
    ...
    if (const auto vv = (s.v3Position - v3Position); D3DXVec3LengthSq (&vv) >= (s.fRadius + m_attribute.fRadius) * (m_attribute.fRadius + s.fRadius))
    {
        return D3DXVECTOR3 (0.0f, 0.0f, 0.0f);
    }
    D3DXVECTOR3 c;
...
void CCylinderCollisionInstance::Render (D3DFILLMODE d3dFillMode)
{
    static CScreen s;
    STATEMANAGER.SetRenderState (D3DRS_TEXTUREFACTOR, 0xffffffff);
    s.RenderCylinder (nullptr, m_attribute.v3Position.x, m_attribute.v3Position.y, m_attribute.v3Position.z + m_attribute.fHeight / 2, m_attribute.fRadius, m_attribute.fHeight,
                      d3dFillMode);
}
...
D3DXVECTOR3 CAABBCollisionInstance::OnGetCollisionMovementAdjust (const CDynamicSphereInstance & s) const
{
    ...
    */
    ...
    if (const auto vv = (v3Temp - s.v3Position); D3DXVec3LengthSq (&vv) < s.fRadius * s.fRadius)
    {
