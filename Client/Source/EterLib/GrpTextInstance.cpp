...
{
    if (CGraphicFontTexture::TCharacterInfomation* pInsCharInfo = pFontTexture->GetCharacterInfomation (codePage, text))
    {
    ...
    while (begin < end)
    {
        ...
        else
        {
                ...
                for (i = wArabicTextLen - 1 ; i >= 0; --i)
                {
                    ...
                    if (isEnglish)
                    {
                        ...
                        else if (Arabic_IsInPresentation (wArabicChar) || Arabic_IsInSymbol (wArabicChar))
                        {
                            ...
                            {
                                if (int ret = GetTextTag (&wArabicText[e], wArabicTextLen - e, len, hyperlinkBuffer); ret == TEXT_TAG_PLAIN || ret == TEXT_TAG_TAG)
                                {
                                    if (hyperlinkStep == 1)
                                    {
            ...
            else    // 아랍외 다른 지역.
            {
                ...
                {
                    if (int ret = GetTextTag (&wText[i], wTextLen - i, len, hyperlinkBuffer); ret == TEXT_TAG_PLAIN || ret == TEXT_TAG_TAG)
                    {
                        if (hyperlinkStep == 1)
                        {
...
void CGraphicTextInstance::Render (RECT * pClipRect)
{
...

    {
        ...
        if (m_isOutline)
        {
            ...
            for (auto i = m_pCharInfoVector.begin(); i != m_pCharInfoVector.end(); ++i)
            {
                pCurCharInfo = *i;
...
WORD CGraphicTextInstance::GetTextLineCount()
{
    ...
    float fx = 0.0f;
    WORD wLineCount = 1;
    for (auto itor = m_pCharInfoVector.begin(); itor != m_pCharInfoVector.end(); ++itor)
    {
        pCurCharInfo = *itor;
