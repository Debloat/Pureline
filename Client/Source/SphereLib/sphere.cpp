                             ...
                             Vector3d *intersect)
{
    ...
    if (float dist2 = EO.x * EO.x + EO.y * EO.y + EO.z * EO.z; dist2 < mRadius2)
    {
        V *= -1;
    }
    ...
    if (float disc = mRadius2 - (EO.Length2() - v * v); disc > 0.0f)
    {
                             ...
                             Vector3d *intersect)
{
    ...
    if (bool hit = RayIntersectionInFront(rayOrigin, V, &sect))
    {
        if (float d = rayOrigin.DistanceSq(sect); d > (distance * distance))
        {
            return false;
        }
        if (intersect)
                                    ...
                                    Vector3d *intersect)
{
    ...
    if (bool hit = RayIntersection(rayOrigin, V, &sect))
    {
