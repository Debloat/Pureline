...
bool CTerrainAccessor::SaveShadowFromD3DTexture8 (const std::string & c_rstrMapName, LPDIRECT3DTEXTURE9 lpShadowTexture)
{
    char szFileName[256];
    DWORD ulID = (DWORD) (m_wX) * 1000L + (DWORD) (m_wY);
...
bool CTerrainAccessor::SaveMiniMapFromD3DTexture8 (const std::string & c_rstrMapName, LPDIRECT3DTEXTURE9 lpMiniMapTexture)
{
    char szFileName[256];
    DWORD ulID = (DWORD) (m_wX) * 1000L + (DWORD) (m_wY);
...
void CTerrainAccessor::RAW_GenerateAttrSplat()
{
    ...
    if (rAttrSplat.NeedsUpdate)
    {
        ...
        do
        {
            hr = ms_lpd3dDevice->CreateTexture (ATTRMAP_XSIZE, ATTRMAP_YSIZE, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &m_lpAttrTexture, nullptr);
        }
        while (FAILED (hr));
        do
        {
