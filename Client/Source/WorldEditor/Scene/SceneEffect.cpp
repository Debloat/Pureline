...
class CPositionChangeEventReceiver : public CEffectTranslationDialog::IEventReciever
{
    public:
        ...
        void OnEvent()
        {
            m_pOwner->OnChangeEffectPosition();
        }
    protected:
...
void CSceneEffect::OnUpdate()
{
    ...
    for (auto itor = m_StartTimeVector.begin(); itor != m_StartTimeVector.end();)
    {
        if (float fTime = *itor; m_fCurrentTime - fTime > m_pEffectAccessor->GetLifeTime())
        {
...
void CSceneEffect::OnRender(BOOL bClear)
{
    ...
    // draw time; iCurrentTime - iLastTime;
    if (int ElapsedTime = iCurrentTime - m_iStartTime; ElapsedTime < 100)
    {
...
void CSceneEffect::RenderPositionGraph()
{
    ...
    for (i = 0; i < iPositionCount; ++i)
    {
        ...
        RenderSphere(nullptr, pos.x, pos.y, pos.z, 4.0f);
...
void CSceneEffect::RenderBoundingSphere()
{
    ...
    RenderSphere(nullptr, c_rv3Pos.x, c_rv3Pos.y, c_rv3Pos.z, fRadius, D3DFILL_WIREFRAME);
    STATEMANAGER.RestoreRenderState(D3DRS_TEXTUREFACTOR);
}
...
void CSceneEffect::RenderEffectPosition()
{
    ...
    for (auto it = m_StartTimeVector.begin(); it != m_StartTimeVector.end(); ++it)
    {
        ...
        RenderSphere(nullptr, pos.x, pos.y, pos.z, 3.0f);
        STATEMANAGER.RestoreRenderState(D3DRS_TEXTUREFACTOR);
    }
}
...
void CSceneEffect::PickingPositionGraph()
{
    ...
    else
    {
        ...
        if ((it = std::ranges::find(GrippedIndexVector, m_dwGrippedPositionIndex)) != GrippedIndexVector.end())
        {
            ++it;
        }
...
{
    auto itor = m_EffectInstanceAccessorList.begin();
...
CSceneEffect::~CSceneEffect()
{
    ...
    m_pEffectTranslationDialog = nullptr;
}
