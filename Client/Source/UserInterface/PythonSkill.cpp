...
static int SplitLine(const char* c_szText, CTokenVector* pstTokenVector, const char* c_szDelimeter)
{
...
static void string_replace_word(const char* base, int base_len, const char* src, int src_len, const char* dst, int dst_len, std::string& result)
{
    result.resize(0);
...
bool CPythonSkill::RegisterSkillTable(const char* c_szFileName)
{
    ...
    for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
    {
        ...
        if (const std::string & c_strSPCostPoly = TokenVector[TABLE_TOKEN_TYPE_SP_COST_POLY]; !c_strSPCostPoly.empty())
        {
            rSkillData.strNeedSPFormula = c_strSPCostPoly;
        }

        if (const std::string & c_strCooldownPoly = TokenVector[TABLE_TOKEN_TYPE_COOLDOWN_POLY]; !c_strCooldownPoly.empty())
        {
            rSkillData.strCoolTimeFormula = c_strCooldownPoly;
        }

        if (const std::string & c_strDurationSPCostPoly = TokenVector[TABLE_TOKEN_TYPE_DURATION_SP_COST_POLY]; !c_strDurationSPCostPoly.empty())
        {
            rSkillData.strContinuationSPFormula = c_strDurationSPCostPoly;
        }

        if (const std::string & c_strTargetRange = TokenVector[TABLE_TOKEN_TYPE_TARGET_RANGE]; !c_strTargetRange.empty())
        {
        ...
        if (const std::string & c_strMaxLevel = TokenVector[TABLE_TOKEN_TYPE_MAX_LEVEL]; !c_strMaxLevel.empty())
        {
        ...
        if (const std::string & c_strLevelLimit = TokenVector[TABLE_TOKEN_TYPE_LEVEL_LIMIT]; !c_strLevelLimit.empty())
        {
...
bool CPythonSkill::RegisterSkillDesc(const char* c_szFileName)
{
    ...
    for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
    {
        ...
        m_SkillDataMap.try_emplace(iSkillIndex /* emplacing a default constructed object */);
        ...
        // Attribute
        {
            ...
            for (auto it = AttributeTokenVector.begin(); it != AttributeTokenVector.end(); ++it)
            {
        ...
        // Weapon
        {
            ...
            for (auto it = WeaponTokenVector.begin(); it != WeaponTokenVector.end(); ++it)
            {
                std::string & rstrToken = *it;
...
bool CPythonSkill::RegisterSkill(DWORD dwSkillIndex, const char* c_szFileName)
{
    ...
    if (std::string strTypeName; TextFileLoader.GetTokenString("type", &strTypeName))
    {
        stl_lowers(strTypeName);
        std::map<std::string, DWORD>::iterator it = m_SkillTypeIndexMap.find(strTypeName);
        if (m_SkillTypeIndexMap.end() == it)
        {
    ...
    if (CTokenVector * pAttributeTokenVector; TextFileLoader.GetTokenVector("attribute", &pAttributeTokenVector))
    {
        for (auto it = pAttributeTokenVector->begin(); it != pAttributeTokenVector->end(); ++it)
        {
            std::string & rstrToken = *it;
    ...
    if (CTokenVector * pNeedWeaponVector; TextFileLoader.GetTokenVector("needweapon", &pNeedWeaponVector))
    {
        for (auto it = pNeedWeaponVector->begin(); it != pNeedWeaponVector->end(); ++it)
        {
            std::string & rstrToken = *it;
    ...
    if (WORD wMotionIndex; TextFileLoader.GetTokenWord("motionindex", &wMotionIndex))
    {
    ...
    if (WORD wMotionIndexForMe; TextFileLoader.GetTokenWord("motionindexforme", &wMotionIndexForMe))
    {
    ...
    m_SkillDataMap.try_emplace(dwSkillIndex, SkillData);
...
void CPythonSkill::TEST()
{
    ...
    for (auto itor = m_SkillDataMap.begin(); itor != m_SkillDataMap.end(); ++itor)
    {
        TSkillData & rSkillData = itor->second;
        ...
        strLine += "\t";
        if (!rSkillData.ConditionDataVector.empty())
        {
        ...
        if (int iPos = strFileName.find_last_of("/", rSkillData.strIconFileName.length()); iPos > 0)
        {
...
{
    m_SkillTypeIndexMap.try_emplace("none", SKILL_TYPE_NONE);
    m_SkillTypeIndexMap.try_emplace("active", SKILL_TYPE_ACTIVE);
    m_SkillTypeIndexMap.try_emplace("support", SKILL_TYPE_SUPPORT);
    m_SkillTypeIndexMap.try_emplace("guild", SKILL_TYPE_GUILD);
    m_SkillTypeIndexMap.try_emplace("horse", SKILL_TYPE_HORSE);
    m_SkillTypeIndexMap.try_emplace("warrior", SKILL_TYPE_ACTIVE);
    m_SkillTypeIndexMap.try_emplace("assassin", SKILL_TYPE_ACTIVE);
    m_SkillTypeIndexMap.try_emplace("sura", SKILL_TYPE_ACTIVE);
    m_SkillTypeIndexMap.try_emplace("shaman", SKILL_TYPE_ACTIVE);
    m_SkillTypeIndexMap.try_emplace("passive", SKILL_TYPE_ACTIVE);

    m_SkillAttributeIndexMap.try_emplace("NEED_POISON_BOTTLE", SKILL_ATTRIBUTE_NEED_POISON_BOTTLE);
    m_SkillAttributeIndexMap.try_emplace("NEED_EMPTY_BOTTLE", SKILL_ATTRIBUTE_NEED_EMPTY_BOTTLE);
    m_SkillAttributeIndexMap.try_emplace("CAN_USE_IF_NOT_ENOUGH", SKILL_ATTRIBUTE_CAN_USE_IF_NOT_ENOUGH);
    m_SkillAttributeIndexMap.try_emplace("FAN_RANGE", SKILL_ATTRIBUTE_FAN_RANGE);
    m_SkillAttributeIndexMap.try_emplace("NEED_CORPSE", SKILL_ATTRIBUTE_NEED_CORPSE);
    m_SkillAttributeIndexMap.try_emplace("NEED_TARGET", SKILL_ATTRIBUTE_NEED_TARGET);
    m_SkillAttributeIndexMap.try_emplace("TOGGLE", SKILL_ATTRIBUTE_TOGGLE);
    m_SkillAttributeIndexMap.try_emplace("WEAPON_LIMITATION", SKILL_ATTRIBUTE_WEAPON_LIMITATION);
    m_SkillAttributeIndexMap.try_emplace("MELEE_ATTACK", SKILL_ATTRIBUTE_MELEE_ATTACK);
    m_SkillAttributeIndexMap.try_emplace("CHARGE_ATTACK", SKILL_ATTRIBUTE_CHARGE_ATTACK);
    m_SkillAttributeIndexMap.try_emplace("USE_HP", SKILL_ATTRIBUTE_USE_HP);
    m_SkillAttributeIndexMap.try_emplace("CAN_CHANGE_DIRECTION", SKILL_ATTRIBUTE_CAN_CHANGE_DIRECTION);
    m_SkillAttributeIndexMap.try_emplace("STANDING_SKILL", SKILL_ATTRIBUTE_STANDING_SKILL);
    m_SkillAttributeIndexMap.try_emplace("ONLY_FOR_ALLIANCE", SKILL_ATTRIBUTE_ONLY_FOR_ALLIANCE);
    m_SkillAttributeIndexMap.try_emplace("CAN_USE_FOR_ME", SKILL_ATTRIBUTE_CAN_USE_FOR_ME);
    m_SkillAttributeIndexMap.try_emplace("ATTACK_SKILL", SKILL_ATTRIBUTE_ATTACK_SKILL);
    m_SkillAttributeIndexMap.try_emplace("MOVING_SKILL", SKILL_ATTRIBUTE_MOVING_SKILL);
    m_SkillAttributeIndexMap.try_emplace("HORSE_SKILL", SKILL_ATTRIBUTE_HORSE_SKILL);
    m_SkillAttributeIndexMap.try_emplace("TIME_INCREASE_SKILL", SKILL_ATTRIBUTE_TIME_INCREASE_SKILL);
    m_SkillAttributeIndexMap.try_emplace("PASSIVE", SKILL_ATTRIBUTE_PASSIVE);
    m_SkillAttributeIndexMap.try_emplace("CANNOT_LEVEL_UP", SKILL_ATTRIBUTE_CANNOT_LEVEL_UP);
    m_SkillAttributeIndexMap.try_emplace("ONLY_FOR_GUILD_WAR", SKILL_ATTRIBUTE_ONLY_FOR_GUILD_WAR);
    m_SkillAttributeIndexMap.try_emplace("CIRCLE_RANGE", SKILL_ATTRIBUTE_CIRCLE_RANGE);
    m_SkillAttributeIndexMap.try_emplace("SEARCH_TARGET", SKILL_ATTRIBUTE_SEARCH_TARGET);

    m_SkillNeedWeaponIndexMap.try_emplace("SWORD", SKILL_NEED_WEAPON_SWORD);
    m_SkillNeedWeaponIndexMap.try_emplace("DAGGER", SKILL_NEED_WEAPON_DAGGER);
    m_SkillNeedWeaponIndexMap.try_emplace("BOW", SKILL_NEED_WEAPON_BOW);
    m_SkillNeedWeaponIndexMap.try_emplace("TWO_HANDED", SKILL_NEED_WEAPON_TWO_HANDED);
    m_SkillNeedWeaponIndexMap.try_emplace("DOUBLE_SWORD", SKILL_NEED_WEAPON_DOUBLE_SWORD);
    m_SkillNeedWeaponIndexMap.try_emplace("BELL", SKILL_NEED_WEAPON_BELL);
    m_SkillNeedWeaponIndexMap.try_emplace("FAN", SKILL_NEED_WEAPON_FAN);
    m_SkillNeedWeaponIndexMap.try_emplace("ARROW", SKILL_NEED_WEAPON_ARROW);
    m_SkillNeedWeaponIndexMap.try_emplace("EMPTY_HAND", SKILL_NEED_WEAPON_EMPTY_HAND);

    m_SkillWeaponTypeIndexMap.try_emplace(std::string("SWORD"), CItemData::WEAPON_SWORD);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("DAGGER"), CItemData::WEAPON_DAGGER);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("BOW"), CItemData::WEAPON_BOW);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("TWO_HANDED"), CItemData::WEAPON_TWO_HANDED);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("DOUBLE_SWORD"), CItemData::WEAPON_DAGGER);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("BELL"), CItemData::WEAPON_BELL);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("FAN"), CItemData::WEAPON_FAN);
    m_SkillWeaponTypeIndexMap.try_emplace(std::string("ARROW"), CItemData::WEAPON_ARROW);

    SSkillData::ms_StatusNameMap.try_emplace(std::string("chain"), POINT_NONE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("HR"), POINT_HIT_RATE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("LV"), POINT_LEVEL);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("Level"), POINT_LEVEL);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MaxHP"), POINT_MAX_HP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MaxSP"), POINT_MAX_SP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MinMWEP"), POINT_MIN_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MaxMWEP"), POINT_MAX_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MinWEP"), POINT_MIN_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MaxWEP"), POINT_MAX_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MinATK"), POINT_MIN_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MaxATK"), POINT_MAX_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("ATKSPD"), POINT_ATT_SPEED);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("AttackPower"), POINT_MIN_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("AtkMin"), POINT_MIN_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("AtkMax"), POINT_MAX_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("DefencePower"), POINT_DEF_GRADE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("DEF"), POINT_DEF_GRADE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("MWEP"), POINT_MAGIC_ATT_GRADE);

    SSkillData::ms_StatusNameMap.try_emplace(std::string("MagicAttackPower"), POINT_MAGIC_ATT_GRADE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("INT"), POINT_IQ);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("STR"), POINT_ST);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("DEX"), POINT_DX);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("CON"), POINT_HT);

    SSkillData::ms_StatusNameMap.try_emplace(std::string("minatk"), POINT_MIN_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxatk"), POINT_MAX_ATK);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("minmtk"), POINT_MIN_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxmtk"), POINT_MAX_WEP);

    // GUILD_SKILL_DISPLAY_BUG_FIX
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxhp"), POINT_MAX_HP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxsp"), POINT_MAX_SP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("odef"), POINT_DEF_GRADE);
    // END_OF_GUILD_SKILL_DISPLAY_BUG_FIX

    SSkillData::ms_StatusNameMap.try_emplace(std::string("minwep"), POINT_MIN_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxwep"), POINT_MAX_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("minmwep"), POINT_MIN_MAGIC_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("maxmwep"), POINT_MAX_MAGIC_WEP);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("lv"), POINT_LEVEL);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("ar"), POINT_HIT_RATE);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("iq"), POINT_IQ);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("str"), POINT_ST);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("dex"), POINT_DX);
    SSkillData::ms_StatusNameMap.try_emplace(std::string("con"), POINT_HT);
    ...
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("atk"), POINT_MIN_ATK);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("mtk"), POINT_MIN_WEP);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("wep"), POINT_MIN_WEP);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("lv"), POINT_LEVEL);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("ar"), POINT_HIT_RATE);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("iq"), POINT_IQ);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("str"), POINT_ST);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("dex"), POINT_DX);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("con"), POINT_HT);

    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("atk"), POINT_MAX_ATK);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("mtk"), POINT_MAX_WEP);
    SSkillData::ms_NewMinStatusNameMap.try_emplace(std::string("wep"), POINT_MAX_WEP);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("lv"), POINT_LEVEL);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("ar"), POINT_HIT_RATE);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("iq"), POINT_IQ);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("str"), POINT_ST);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("dex"), POINT_DX);
    SSkillData::ms_NewMaxStatusNameMap.try_emplace(std::string("con"), POINT_HT);

    m_PathNameMap.try_emplace(std::string("WARRIOR"), "warrior");
    m_PathNameMap.try_emplace(std::string("ASSASSIN"), "assassin");
    m_PathNameMap.try_emplace(std::string("SURA"), "sura");
    m_PathNameMap.try_emplace(std::string("SHAMAN"), "shaman");
    m_PathNameMap.try_emplace(std::string("PASSIVE"), "passive");
    m_PathNameMap.try_emplace(std::string("SUPPORT"), "common/support");
    m_PathNameMap.try_emplace(std::string("GUILD"), "common/guild");
    m_PathNameMap.try_emplace(std::string("HORSE"), "common/horse");
}
...
BOOL CPythonSkill::SSkillData::CanUseWeaponType(DWORD dwWeaponType) const
{
...
{
    std::map<std::string, DWORD>* pStatusNameMap = nullptr;
    ...
    if (std::map<std::string, DWORD>::iterator it = pStatusNameMap->find(c_szStateName); it != pStatusNameMap->end())
    {
...
const char* CPythonSkill::SSkillData::GetAffectDescription(DWORD dwIndex, float fSkillLevel)
{
    ...
    if (CP_ARABIC == ::GetDefaultCodePage())
    {
        ...
        if (int first = strDescription.find("%.0f"); first >= 0)
        {
...
DWORD CPythonSkill::SSkillData::GetSkillMotionIndex(int iGrade) const
{
...
BYTE CPythonSkill::SSkillData::GetMaxLevel() const
{
    return byMaxLevel;
}

BYTE CPythonSkill::SSkillData::GetLevelUpPoint() const
{
...
static PyObject* skillSetPathName(PyObject * poSelf, PyObject * poArgs)
{
    char* szPathName;
...
static PyObject* skillRegisterSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
...
static PyObject* skillLoadSkillData(PyObject * poSelf, PyObject * poArgs)
{
...
static PyObject* skillRegisterSkillDesc(PyObject * poSelf, PyObject * poArgs)
{
    char* szFileName;
...
static PyObject* skillClearSkillData(PyObject * poSelf, PyObject * poArgs)
{
    CPythonSkill::SSkillData::ms_dwTimeIncreaseSkillNumber = 0;
    return Py_BuildNone();
}

static PyObject* skillGetSkillName(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillDescription(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillType(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillConditionDescriptionCount(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillConditionDescription(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillAffectDescriptionCount(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillAffectDescription(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillCoolTime(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillNeedSP(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillContinuationSP(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillMaxLevel(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillLevelUpPoint(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillLevelLimit(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillIsSkillRequirement(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillRequirementData(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillRequireStatCount(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetSkillRequireStatData(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillCanLevelUpSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillCheckRequirementSueccess(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetNeedCharacterLevel(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillIsToggleSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillIsUseHPSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillIsStandingSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillCanUseSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillIsLevelUpSkill(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
    ...
    if (CPythonSkill::SSkillData * c_pSkillData; !CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
    {
...
static PyObject* skillGetIconName(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetIconImage(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetIconInstance(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetIconImageNew(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetIconInstanceNew(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillDeleteIconInstance(PyObject * poSelf, PyObject * poArgs)
{
    int iHandle;
...
static PyObject* skillGetGradeData(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetNewAffectDataCount(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetNewAffectData(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillGetDuration(PyObject * poSelf, PyObject * poArgs)
{
    int iSkillIndex;
    if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
    {
...
static PyObject* skillTEST(PyObject * poSelf, PyObject * poArgs)
{
