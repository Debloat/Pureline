...
bool CPythonNetworkStream::RecvRefineInformationPacket()
{
    ...
    {
        PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AppendMaterialToRefineDialog", Py_BuildValue("(ii)", rkRefineTable.materials[i].vnum,
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
    ...
    PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_NEW_AddAffect", Py_BuildValue("(iiii)", rkElement.dwType, rkElement.bPointIdxApplyOn, rkElement.lApplyValue,
                                                                                                  rkElement.lDuration));
...
bool CPythonNetworkStream::RecvViewEquipPacket()
{
    ...
    for (int i = 0; i < WEAR_MAX_NUM; ++i)
    {
        ...
        {
            PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetEquipmentDialogAttr", Py_BuildValue("(iiiii)", kViewEquipPacket.dwVID, i, k, rItemSet.aAttr[k].bType,
                                                                                                            rItemSet.aAttr[k].sValue));
        }
    }
