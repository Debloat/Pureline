...
void CPythonCharacterManager::InsertPVPKey(DWORD dwVIDSrc, DWORD dwVIDDst)
{
    ...
    if (CInstanceBase* pkInstSrc = GetInstancePtr(dwVIDSrc))
    {
...
void CPythonCharacterManager::UpdateTransform()
{
    ...
    if (CInstanceBase * pMainInstance = GetMainInstancePtr())
    {
...
void CPythonCharacterManager::__RenderSortedAliveActorList()
{
    ...
    for (auto i = rkMap_pkInstAlive.begin(); i != rkMap_pkInstAlive.end(); ++i)
    {
...
void CPythonCharacterManager::__RenderSortedDeadActorList()
{
    ...
    for (auto i = rkLst_pkInstDead.begin(); i != rkLst_pkInstDead.end(); ++i)
    {
...
void CPythonCharacterManager::__NEW_Pick()
{
    ...
    if (pkInstMain)
        if (pkInstMain->CanPickInstance())
            if (m_kVct_pkInstPicked.end() != std::ranges::find(m_kVct_pkInstPicked, pkInstMain))
            {
                if (m_pkInstPick)
...
CInstanceBase* CPythonCharacterManager::GetCloseInstance(CInstanceBase * pInstance)
{
    ...
    for (; itor != m_kAliveInstMap.end(); ++itor)
    {
        ...
        if (DWORD dwVirtualNumber = pTargetInstance->GetVirtualNumber(); CPythonNonPlayer::ON_CLICK_EVENT_BATTLE != CPythonNonPlayer::Instance().GetEventType(dwVirtualNumber))
        {
            continue;
        }
