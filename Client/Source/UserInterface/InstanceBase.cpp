...
{
    if (const TPixelPosition& c_rkPPosCur = m_GraphicThingInstance.NEW_GetCurPixelPositionRef();
        CPythonBackground::Instance().isAttrOn (c_rkPPosCur.x, c_rkPPosCur.y, CTerrainImpl::ATTRIBUTE_BANPK))
    {
        return TRUE;
    }
...
bool CInstanceBase::Create (const SCreateData& c_rkCreateData)
{
    ...
    if (const int c_iGuildSymbolRace = 14200; c_iGuildSymbolRace == GetRace())
    {
...
void CInstanceBase::__Create_SetWarpName (const SCreateData& c_rkCreateData)
{
    ...
    if (CPythonNonPlayer::Instance().GetName (c_rkCreateData.m_dwRace, &c_szName))
    {
        ...
        if (int iFindingPos = strName.find_first_of (" ", 0); iFindingPos > 0)
        {
...
void CInstanceBase::StateProcess()
{
    while (1)
    {
        ...
        if (DWORD dwCurChkTime = ELTimer_GetServerFrameMSec(); dwCurChkTime < dwDstChkTime)
        {
            return;
        }
...
void CInstanceBase::__ProcessFunctionEmotion (DWORD dwMotionNumber, DWORD dwTargetVID, const TPixelPosition & c_rkPosDst)
{
    ...
    if (CInstanceBase * pTargetInstance = CPythonCharacterManager::Instance().GetInstancePtr (dwTargetVID))
    {
        ...
        auto wMotionNumber1 = HIWORD (dwMotionNumber);
        auto wMotionNumber2 = LOWORD (dwMotionNumber);
...
void CInstanceBase::RenderToShadowMap()
{
    ...
    if (float fDistance = NEW_GetDistanceFromDestInstance (*pkInstMain); fDistance >= SHADOW_APPLY_DISTANCE)
    {
        return;
    }
...
{
    if (DWORD dwShape; __ArmorVnumToShape (dwArmor, &dwShape))
    {
        CItemData * pItemData;
...
bool CInstanceBase::SetWeapon (DWORD eWeapon)
{
    ...
    //Weapon Effect
    if (CItemData * pItemData; CItemManager::Instance().GetItemDataPointer (eWeapon, &pItemData))
    {
        __GetRefinedEffect (pItemData);
    }
    else
...
void CInstanceBase::RefreshState (DWORD dwMotIndex, bool isLoop)
{
    ...
    if (CItemData * pItemData; rkItemMgr.GetItemDataPointer (dwPartItemID, &pItemData))
    {
