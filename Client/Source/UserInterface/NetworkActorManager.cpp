...
void CNetworkActorManager::__OLD_Update()
{
    ...
    for (auto i = m_kNetActorDict.begin(); i != m_kNetActorDict.end(); ++i)
    {
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor (SNetworkActorData& rkNetActorData)
{
    ...
    if (CInstanceBase* pOldInstance = rkChrMgr.GetInstancePtr (dwVID))
    {
        TPixelPosition oldPos;
        pOldInstance->GetGraphicThingInstancePtr()->GetPixelPosition (&oldPos);

        if (bool bIsMountingHorse = pOldInstance->IsMountingHorse(); (bIsMountingHorse && kCreateData.m_dwMountVnum == 0) ||
    ...
    if (DWORD dwElapsedTime = dwClientCurTime - rkNetActorData.m_dwClientSrcTime; dwElapsedTime < rkNetActorData.m_dwDuration)
    {
...
void CNetworkActorManager::AppendActor (const SNetworkActorData& c_rkNetActorData)
{
    if (__IsMainActorVID (c_rkNetActorData.m_dwVID))
    {
        ...
        if (CInstanceBase* pMainInstance = CPythonCharacterManager::Instance().GetInstancePtr (c_rkNetActorData.m_dwVID))
        {
...
void CNetworkActorManager::MoveActor (const SNetworkMoveActorData& c_rkNetMoveActorData)
{
    ...
    if (CInstanceBase* pkInstFind = __FindActor (rkNetActorData, c_rkNetMoveActorData.m_lPosX, c_rkNetMoveActorData.m_lPosY))
    {
        TPixelPosition kPPosDst;
...
void CNetworkActorManager::SyncActor (DWORD dwVID, LONG lPosX, LONG lPosY)
{
    ...
    if (CInstanceBase* pkInstFind = __FindActor (rkNetActorData))
    {
