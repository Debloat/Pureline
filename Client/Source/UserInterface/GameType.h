...
enum EDragonSoulStepTypes
{
    ...
    19; // [주의] 숫자(19) 하드코딩 주의. 현재 서버에서 코스츔 슬롯은 19부터임. 서버 common/length.h 파일의 EWearPositions 열거형 참고.
...
using TItemPos = struct SItemPos
{
    BYTE window_type;
    WORD cell;
    SItemPos()
...
using TItemData = struct packet_item
{
    DWORD       vnum;
    BYTE        count;
    DWORD       flags;
...
using TShopItemData = struct packet_shop_item
{
    DWORD       vnum;
    DWORD       price;
    BYTE        count;
