...
{
    m_ChatSetMap.try_emplace(dwID /* emplacing a default constructed object */);
    return dwID;
}
...
void CWhisper::AppendChat(int iType, const char* c_szChat)
{
    ...
    #if defined(LOCALE_SERVICE_YMIR)
    CGraphicText* pkDefaultFont = static_cast<CGraphicText*> (DefaultFont_GetResource());
    #else
    CGraphicText* pkDefaultFont = (iType == CPythonChat::WHISPER_TYPE_GM) ? static_cast<CGraphicText*> (DefaultItalicFont_GetResource()) : static_cast<CGraphicText*>
