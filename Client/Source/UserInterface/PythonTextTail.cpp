...
void CPythonTextTail::ArrangeTextTail()
{
    ...
    for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
    {
        ...
        if (CGraphicTextInstance * pGuildNameInstance = pTextTail->pGuildNameTextInstance; pMarkInstance && pGuildNameInstance)
        {
            int iWidth;
            int iHeight;
...
void CPythonTextTail::ShowItemTextTail (DWORD VirtualID)
{
    ...
    if (m_ItemTextTailList.end() != std::ranges::find (m_ItemTextTailList, pTextTail))
    {
        //Tracef("이미 리스트에 있음 : %d\n", VirtualID);
        return;
    }
...
CPythonTextTail::TTextTail* CPythonTextTail::RegisterTextTail (DWORD dwVirtualID, const char* c_szText, CGraphicObjectInstance * pOwner, float fHeight,
