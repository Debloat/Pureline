...
DWORD CGuildMarkManager::__AllocMarkID(DWORD guildID)
{
    ...
    // 이미지가 없다면 만들기 위해
    if (CGuildMarkImage* pkImage = __GetImage(imgIdx); pkImage && AddMarkIDByGuildID(guildID, markID))
    {
        return markID;
    }
...
DWORD CGuildMarkManager::SaveMark(DWORD guildID, BYTE * pbMarkImage)
{
    ...
    if (CGuildMarkImage * pkImage = __GetImage(imgIdx))
    {
...
void CGuildMarkManager::DeleteMark(DWORD guildID)
{
    ...
    if (CGuildMarkImage * pkImage; (pkImage = __GetImage(it->second / CGuildMarkImage::MARK_TOTAL_COUNT)) != nullptr)
    {
