...
void __SetWeaponPower(IAbstractPlayer& rkPlayer, DWORD dwWeaponID)
{
    ...
    if (CItemData * pkWeapon; CItemManager::Instance().GetItemDataPointer(dwWeaponID, &pkWeapon))
    {
...
bool CPythonNetworkStream::RecvCharacterAppendPacket()
{
    ...
    {
        const char* c_szName;
        if (CPythonNonPlayer& rkNonPlayer = CPythonNonPlayer::Instance(); rkNonPlayer.GetName(kNetActorData.m_dwRace, &c_szName))
        {
