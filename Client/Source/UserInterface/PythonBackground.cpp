...
bool CPythonBackground::SetShadowLevel (int eLevel)
{
    ...
    switch (m_eShadowLevel)
    {
        ...
        case SHADOW_ALL_MAX:
            rkMap.SetDrawShadow (true);
            rkMap.SetDrawCharacterShadow (true);
            rkMap.SetShadowTextureSize (16384);
            break;
    }
...
{
    if (static int s_isCreateProperty = false; !s_isCreateProperty)
    {
...
void CPythonBackground::RenderCharacterShadowToTexture()
{
    ...
    m_eShadowLevel == SHADOW_GROUND_AND_SOLO)
    {
        ...
        if (bool canRender = rkMap.BeginRenderCharacterShadowToTexture())
        {
            ...
            else
            {
                ...
                /* - DYNAMIC_TREE_SHADOWS ------------------------------ */
                if (D3DXVECTOR3 vSunDir; GetLightDirection (vSunDir))
                {
