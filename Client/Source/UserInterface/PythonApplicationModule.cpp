...
PyObject* appGetFileList(PyObject* poSelf, PyObject* poArgs)
{
    ...
    if (HANDLE hFind = FindFirstFile(szFilter, &wfd); hFind != INVALID_HANDLE_VALUE)
    {
        do
        {
...
PyObject* appCreate(PyObject* poSelf, PyObject* poArgs)
{
    ...
    if (CPythonApplication& rkApp = CPythonApplication::Instance(); !rkApp.Create(poSelf, szName, width, height, Windowed))
    {
        //return Py_BuildNone();
        return nullptr;
    }
...
PyObject* appSetCameraSpeed(PyObject * poSelf, PyObject * poArgs)
{
    ...
    if (CCamera * pCamera = CCameraManager::Instance().GetCurrentCamera())
    {
