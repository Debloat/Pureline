...
{
    if (float fDistance = NEW_GetDistanceFromDestPixelPosition (c_rkPPosDst); fDistance > 150.0f)
    {
        return FALSE;
    }
...
BOOL CInstanceBase::CheckAdvancing()
{
    ...
    // Check
    if (D3DXVECTOR3 v3NextPosition = rv3Position + rv3MoveDirection; rkBG.isAttrOn (v3NextPosition.x, -v3NextPosition.y, CTerrainImpl::ATTRIBUTE_BLOCK))
    {
        BlockMovement();
        return TRUE;
    }
