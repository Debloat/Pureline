...
{
    if (DWORD dwPVPKey = __GetPVPKey(dwVIDSrc, dwVIDDst); !g_kSet_dwPVPKey.contains(dwPVPKey))
    {
        return false;
    }
...
{
    if (DWORD dwPVPKey = __GetPVPKey(dwVIDSrc, dwVIDDst); !g_kSet_dwPVPReadyKey.contains(dwPVPKey))
    {
        return false;
    }
...
{
    if (DWORD dwGVGKey = __GetPVPKey(dwSrcGuildID, dwDstGuildID); !g_kSet_dwGVGKey.contains(dwGVGKey))
    {
        return false;
    }
...
{
    if (DWORD dwDUELKey = __GetPVPKey(dwVIDSrc, dwVIDDst); !g_kSet_dwDUELKey.contains(dwDUELKey))
    {
        return false;
    }
...
UINT CInstanceBase::GetNameColorIndex()
{
    if (IsPC())
    {
        ...
        if (IAbstractPlayer& rPlayer = IAbstractPlayer::GetSingleton(); rPlayer.IsPartyMemberByVID(GetVirtualID()))
        {
...
bool CInstanceBase::RegisterEffect(UINT eEftType, const char* c_szEftAttachBone, const char* c_szEftName, bool isCache)
{
    ...
    if (DWORD& rdwCRCEft = ms_adwCRCAffectEffect[eEftType]; !CEffectManager::Instance().RegisterEffect2(c_szEftName, &rdwCRCEft, isCache))
    {
...
{
    g_TitleNameMap.try_emplace(iIndex, c_szTitleName);
}
