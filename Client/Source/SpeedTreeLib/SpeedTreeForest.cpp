...
void CSpeedTreeForest::UpdateSystem(float fCurrentTime)
{
    ...
    SetupWindMatrices(fElapsedTime);
}
...
void CSpeedTreeForest::SetupWindMatrices(float fDeltaTime)
{
    ...
    {
        afMatrixTimes[i] += fDeltaTime;
    }

    // compute maximum branch throw
    /* - YOSUN_CONTROL_CENTER ------------------------------ */
    float fBaseAngle = m_fWindStrength * YosunControlCenter::instance().fWindStrength;
    /* ----------------------------------------------------- */
    ...
    for (int j = 0; j < c_nNumWindMatrices; ++j)
    {
        ...
        /* - YOSUN_CONTROL_CENTER ------------------------------ */
        float fBaseFreq = m_fWindStrength * YosunControlCenter::instance().windOscillationSpeed;
        /* ----------------------------------------------------- */
