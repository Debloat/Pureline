...
void CGrannyModelInstance::Update(DWORD dwAniFPS)
{
    ...
    const auto c_dwCurUpdateFrame = (DWORD)(GetLocalTime() * static_cast<float> (ANIFPS_MAX));
    if (const DWORD ANIFPS_STEP = ANIFPS_MAX / dwAniFPS; c_dwCurUpdateFrame > ANIFPS_STEP && c_dwCurUpdateFrame / ANIFPS_STEP == m_dwOldUpdateFrame / ANIFPS_STEP)
    {
        return;
    }
