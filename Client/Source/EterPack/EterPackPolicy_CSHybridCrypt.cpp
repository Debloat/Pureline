...
bool EterPackPolicy_CSHybridCrypt::GenerateSupplementaryDataBlock(std::string& rfilename, const std::string& strMapName, IN const BYTE* pSrcData, IN int iSrcLen,
                                                                  OUT LPBYTE& pDestData, OUT int& iDestLen)
{
    #ifdef __THEMIDA__
    VM_START
    #endif
    ...
    if (TSupplementaryDataBlockMap::const_iterator cit = m_mapSDBMap.find(dwFileNameHash); cit != m_mapSDBMap.end())
    {
        //TODO : log already registered
        return false;
    }
...
void EterPackPolicy_CSHybridCrypt::WriteSupplementaryDataBlockToFile(CFileBase& rFile)
{
    ...
    for (TSupplementaryDataBlockMap::const_iterator cit = m_mapSDBMap.begin(); cit != m_mapSDBMap.end(); ++cit)
    {
        ...
        auto bSDBSize = (BYTE)(sdbVector.size());
