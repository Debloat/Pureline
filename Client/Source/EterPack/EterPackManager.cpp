...
{
    if (std::string strFileName; 0 == ConvertFileName(c_szPathName, strFileName))
    {
        return &m_RootPack;
    }
    else
    {
        for (auto itor = m_DirPackMap.begin(); itor != m_DirPackMap.end(); ++itor)
        {
...
void CEterPackManager::LoadStaticCache(const char* c_szFileName)
{
    ...
    if (std::unordered_map<DWORD, SCache>::iterator f = m_kMap_dwNameKey_kCache.find(dwFileNameHash); m_kMap_dwNameKey_kCache.end() != f)
    {
        return;
    }
    ...
    m_kMap_dwNameKey_kCache.try_emplace(dwFileNameHash, kNewCache);
}
...
{
    for (auto i = m_kMap_dwNameKey_kCache.begin(); i != m_kMap_dwNameKey_kCache.end(); ++i)
    {
...
bool CEterPackManager::GetFromPack(CMappedFile & rMappedFile, const char* c_szFileName, LPCVOID * pData)
{
    ...
    else
    {
        ...
        if (SCache* pkCache = __FindCache(dwFileNameHash))
        {
...
{
    if (std::string strFileName; 0 == ConvertFileName(c_szFileName, strFileName))
    {
...
bool CEterPackManager::RegisterPackWhenPackMaking(const char* c_szName, const char* c_szDirectory, CEterPack* pPack)
{
    ...
    m_DirPackMap.try_emplace(c_szDirectory, pPack);
    return true;
}
...
bool CEterPackManager::RegisterPack(const char* c_szName, const char* c_szDirectory, const BYTE* c_pbIV)
{
    ...
    if (c_szDirectory && c_szDirectory[0] != '*')
    {
        ...
        if (m_DirPackMap.end() == itor)
        {
            ...
            m_DirPackMap.try_emplace(c_szDirectory, pEterPack);
        }
    }
...
CEterPackManager::CEterPackManager() : m_isCacheMode(false), m_iSearchMode(SEARCH_FILE_FIRST)
{
...
CEterPackManager::~CEterPackManager()
{
    ...
    auto i = m_PackMap.begin();
    auto e = m_PackMap.end();
    while (i != e)
    {
        delete i->second;
        i++;
...
void CEterPackManager::RetrieveHybridCryptPackKeys(const BYTE *pStream)
{
    ...
    for (int i = 0; i < iPackageCnt; ++i)
    {
        ...
        for (TEterPackMap::const_iterator cit = m_PackMap.begin(); cit != m_PackMap.end(); ++cit)
        {
            ...
            if (dwPackageNameHash == stringhash().GetHash(noPathName))
            {
                ...
                if (int iHavedCryptKeySize = pCryptPolicy->ReadCryptKeyInfoFromStream(pStream + iMemOffset); iRecvedCryptKeySize != iHavedCryptKeySize)
                {
                    TraceError("CEterPackManager::RetrieveHybridCryptPackKeys	cryptokey length of file(%s) is not matched. received(%d) != haved(%d)", noPathName.c_str(), iRecvedCryptKeySize,
                               iHavedCryptKeySize);
                }
                break;
            }
