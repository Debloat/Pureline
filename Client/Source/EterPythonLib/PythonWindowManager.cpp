...
namespace UI
{
    ...
    void CWindowManager::RunKeyDown (int vkey)
    {
        ...
        if (pKeyCaptureWindow)
            if (m_ReserveDeleteWindowList.end() == std::ranges::find (m_ReserveDeleteWindowList, pKeyCaptureWindow))
            {
    ...
    {
        if (TKeyCaptureWindowMap::iterator itor = m_KeyCaptureWindowMap.find (vkey); m_KeyCaptureWindowMap.end() != itor)
        {
            if (CWindow * pKeyCaptureWindow = itor->second)
            {
