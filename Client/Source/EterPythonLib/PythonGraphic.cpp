...
float CPythonGraphic::GetOrthoDepth() const
{
...
void CPythonGraphic::SetInterfaceRenderState()
{
    ...
    /* - YOSUN_CONTROL_CENTER ------------------------------ */
    if (YosunControlCenter::interfaceAnisotropy)
    {
        ...
        STATEMANAGER.SetSamplerState(0, D3DSAMP_MAXANISOTROPY, YosunControlCenter::anisotropyLevel);
    }
    else
        /* ----------------------------------------------------- */
    {
...
void CPythonGraphic::RestoreViewport() const
{
...
bool CPythonGraphic::SaveScreenShot(const char* c_pszFileName)
{
    ...
    if (FAILED(hr = lpSurface->LockRect(&lockRect, nullptr, D3DLOCK_NO_DIRTY_UPDATE | D3DLOCK_READONLY | D3DLOCK_NOSYSLOCK)))
    {
    ...
    auto* pbyBuffer = new BYTE[uWidth * uHeight * 3];
    ...
    auto* pbySource = (BYTE*) lockRect.pBits;
...
void CPythonGraphic::RenderImage(CGraphicImageInstance* pImageInstance, float x, float y)
{
    ...
    auto width = (float) pImageInstance->GetWidth();
    auto height = (float) pImageInstance->GetHeight();
...
void CPythonGraphic::RenderAlphaImage(CGraphicImageInstance* pImageInstance, float x, float y, float aLeft, float aRight)
{
    ...
    auto DiffuseColor1 = D3DXCOLOR(1.0f, 1.0f, 1.0f, aLeft);
    auto DiffuseColor2 = D3DXCOLOR(1.0f, 1.0f, 1.0f, aRight);
...
/* - IMGUI --------------------------------------------- */
#ifdef _DEBUG
LPDIRECT3DDEVICE9 CPythonGraphic::GetD3DDevice()
{
