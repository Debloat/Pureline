...
class OptionDialog(ui.ScriptWindow):
	...
	def __Load(self):
		...
		# Shadow slider
		cur_level = int(systemSetting.GetShadowLevel())
		cur_level = max(0, min(5, cur_level))
		self.ctrlShadowQuality.SetSliderPos(cur_level / 5.0)
	...
	def OnChangeShadowQuality(self):
		pos = self.ctrlShadowQuality.GetSliderPos()   # expected in [0.0, 1.0]
		level = int(round(pos * 5))                   # 0..5, uses symmetric thresholds
		level = max(0, min(5, level))                 # clamp for safety

		# Snap the slider to the exact tick so bins feel consistent
		snapped = level / 5.0
		if abs(pos - snapped) > 1e-6:
			self.ctrlShadowQuality.SetSliderPos(snapped)

		# Avoid redundant writes
		if level != systemSetting.GetShadowLevel():
			systemSetting.SetShadowLevel(level)
